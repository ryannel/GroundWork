...
### Added (engine wave — honesty scan, 2026-07-03)

`groundwork honesty scan --bet <slug> [--json]` — the computable half of the milestone
honesty audit, diffed against the sealed baseline (`bet/<slug>/approved`): deleted or
renamed-away test guards under `tests/bets/<slug>/` plus test-function-count drops
(labelled heuristic), hand-edits inside generated files (`@generated` / `DO NOT EDIT` /
`Code generated by` markers), and best-effort zero-caller exported symbols in changed
source files (capped at 20, labelled `[best-effort]`; JS/TS, Python, Go — unrecognized
languages skip silently). Exit 0 clean / 1 leads / 2 cannot-run (no tag, not a git
repo). Findings are leads for the honesty-audit agent, never verdicts. Logic in
`lib/bet-honesty/`; the delivery milestone-close prose and the acceptance-auditor brief
