...
The scaffolded Fumadocs site (`docs-site` generator) consumed none of the brand system GroundWork already generates, drew no diagrams, and rendered its sidebar in raw filesystem order patched by a JS hack. It now wears the project's brand, renders Mermaid at build time, orders the canonical doc set declaratively, and opens on a generated landing instead of a redirect (plan: `docs/plans/docs-quality-uplift.md`, WS-F + WS-G).
...
- **Build-time Mermaid.** `source.config.ts` runs `rehype-mermaid` (`strategy: 'inline-svg'`) in the MDX rehype pipeline, with `rehype-mermaid` + `playwright` added to the site's `package.json`. Fenced ` ```mermaid ` blocks render to static SVG at build time — the same plain-Markdown block GitHub renders natively, so content stays dual-render.
...
- **New `groundwork-docs-uplift` maintenance skill.** The companion to the generator: it brings an *existing* doc site — one that predates the branding, was hand-built, or has drifted — to the same target state (brand theme, build-time diagrams, ordered nav, landing page), and gives the docs a reader-first pass that strips any leftover `## Summary for Downstream` residue. Routed from the orchestrator's anytime lane.
