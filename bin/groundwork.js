...
function printHelp() {
  ...
  \x1b[36mgenerate-views\x1b[0m
            Write the generated architecture views to docs/architecture/generated/: services.md
            (service inventory from docker-compose + the code map), modules.md (module inventory +
            the module-graph diagram), contracts.md (contract locations from the code map and the
            captured docs/architecture/api/ record). Hand edits to these files are discarded on the
            next run — the hand-kept rulings live in docs/architecture/index.md.
...
function checkGroundWork() {
  ...
  // Generated architecture views — advisory on the same terms: regenerating is cheap.
  reportGeneratedViewsStatus(p);
...
// ─── generate-views ─────────────────────────────────────────────────────────
// Inventory the code can answer for itself: services, modules, contract
// locations. The rulings in docs/architecture/index.md stay hand-kept; these
// views land as sibling files under docs/architecture/generated/ so the update
// lane's whole-file classification and `check`'s freshness rule stay clean.
// Sources: repo-map + docker-compose + the captured docs/architecture/api/ record.

const GENERATED_VIEWS_DIR = path.join('docs', 'architecture', 'generated');
const GENERATE_VIEWS_COMMAND = 'npx groundwork-method generate-views';
const GENERATED_VIEWS_FILES = ['services.md', 'modules.md', 'contracts.md'];

function generatedViewsFrontmatter(headSha) {
  return [
    '---',
    'generated: true',
    `generated_at_commit: ${headSha || 'unknown'}`,
    `command: ${GENERATE_VIEWS_COMMAND}`,
    '---',
    '',
    `Generated by \`${GENERATE_VIEWS_COMMAND}\` — hand edits are discarded by the next run.`,
    'Decisions and design intent belong in `docs/architecture/index.md`, which is hand-kept.',
    '',
    '',
  ].join('\n');
}
...
// Every mapped source file appears in `centrality`, so it is the map's complete
// file list — group it by top-level partition to answer "what language is this?".
function languagesByPartition(engine, map) {
  const byPartition = new Map();
  for (const entry of map.centrality || []) {
    const top = entry.file.includes('/') ? entry.file.slice(0, entry.file.indexOf('/')) : '.';
    const lang = engine.langForFile(entry.file);
    if (!lang) continue;
    const counts = byPartition.get(top) || new Map();
    counts.set(lang, (counts.get(lang) || 0) + 1);
    byPartition.set(top, counts);
  }
  const out = new Map();
  for (const [top, counts] of byPartition) {
    out.set(
      top,
      [...counts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).map(([l]) => l)
    );
  }
  return out;
}

// Language for a root path, read from the partition the path sits in.
function languageForRoot(langsByPartition, root) {
  if (!root) return '—';
  const top = root.replace(/^\.\//, '').split('/')[0] || '.';
  const langs = langsByPartition.get(top);
  return langs && langs.length ? langs.join(', ') : '—';
}

// Compose is the only place a service that owns no source directory (a broker, a
// vendored container) is declared, so it is read first and the map fills in the rest.
function readComposeServices(targetDir) {
  const files = fs
    .readdirSync(targetDir, { withFileTypes: true })
    .filter((e) => e.isFile() && /^docker-compose.*\.ya?ml$/.test(e.name))
    .map((e) => e.name)
    .sort();
  ...
  try {
    ...
    return { files, services: [], unparsed: true };
  }
  ...
  for (const file of files) {
    let doc;
    try {
      doc = YAML.parse(fs.readFileSync(path.join(targetDir, file), 'utf8'));
    } catch {
      continue; // a malformed compose file degrades to "not read", never a crash
    }
    for (const [name, def] of Object.entries((doc && doc.services) || {})) {
      const build = def && def.build;
      const root = typeof build === 'string' ? build : (build && build.context) || null;
      const ports = ((def && def.ports) || [])
        .map((p) => String(typeof p === 'object' && p ? p.published || '' : p).split(':')[0])
        .filter(Boolean);
      services.push({ name, root, ports, file });
    }
  }
  return { files, services, unparsed: false };
}

function renderUnmappedNote(map) {
  if (!map.unmapped || !map.unmapped.length) return '';
  const rows = map.unmapped
    .map((u) => `- ${u.language} — ${u.files} file(s); ${u.reason}`)
    .join('\n');
  return `\n## Languages the code map does not cover\n\nThese files are absent from every count above:\n\n${rows}\n`;
}

function renderServicesView(map, langsByPartition, compose) {
  const lines = ['# Service Inventory', ''];
  if (compose && compose.services.length) {
    lines.push(
      `Read from ${compose.files.map((f) => `\`${f}\``).join(', ')}, with each service's language read from the code map.`,
      '',
      '| Service | Language | Port | Root path |',
      '|---|---|---|---|'
    );
    for (const s of compose.services.sort((a, b) => a.name.localeCompare(b.name))) {
      lines.push(
        `| ${s.name} | ${languageForRoot(langsByPartition, s.root)} | ${s.ports.join(', ') || '—'} | ${s.root || '—'} |`
      );
    }
  } else {
    const why = compose
      ? `${compose.files.map((f) => `\`${f}\``).join(', ')} declares no services`
      : 'no docker-compose file exists in this repo';
    lines.push(
      `No compose services — ${why}, so this inventory derives from the code map's top-level partitions alone.`,
      'A service that owns no source directory (a broker, a vendored container, a service still planned) is invisible here; the topology in `docs/architecture/index.md` is where it is recorded.',
      '',
      '| Partition | Language | Files |',
      '|---|---|---|'
    );
    for (const m of map.modules || []) {
      const langs = langsByPartition.get(m.path);
      lines.push(`| ${m.path} | ${langs && langs.length ? langs.join(', ') : '—'} | ${m.files} |`);
    }
  }
  if (compose && compose.unparsed) {
    lines.push('', 'The compose file was found but could not be parsed — the YAML parser is unavailable in this install.');
  }
  return lines.join('\n') + '\n' + renderUnmappedNote(map);
}

function renderModulesView(engine, map, langsByPartition) {
  const lines = ['# Module Inventory', ''];
  const coverage = Object.entries(map.coverage || {})
    .map(([lang, c]) => `${lang} ${c.files} file(s) at ${c.fidelity} fidelity`)
    .join('; ');
  lines.push(`Read from the code map. Coverage: ${coverage || 'no mapped source files'}.`, '');
  lines.push('| Partition | Language | Files |', '|---|---|---|');
  for (const m of map.modules || []) {
    const langs = langsByPartition.get(m.path);
    lines.push(`| ${m.path} | ${langs && langs.length ? langs.join(', ') : '—'} | ${m.files} |`);
  }

  const mg = map.module_graph || { modules: [], edges: [] };
  lines.push('', '## Declared modules');
  if (mg.modules.length) {
    lines.push('', '| Module | Kind | Ecosystem | Path |', '|---|---|---|---|');
    for (const m of mg.modules) {
      lines.push(`| ${m.name} | ${m.kind} | ${m.ecosystem} | ${m.path || '—'} |`);
    }
    lines.push('', '```mermaid', engine.renderModuleGraphMermaid(mg).trimEnd(), '```');
  } else {
    lines.push('', `No module graph — ${mg.reason || 'no build manifests were recognised'}.`);
  }
  return lines.join('\n') + '\n' + renderUnmappedNote(map);
}

function renderContractsView(targetDir, map) {
  const lines = ['# Contract Locations', ''];
  lines.push('Where the machine-readable contracts live. The contracts themselves are the source of truth; this is only the index.', '');

  lines.push('## Detected in the code');
  const detected = map.contracts || [];
  lines.push('');
  if (detected.length) for (const f of detected) lines.push(`- \`${f}\``);
  else lines.push('No contract or spec files detected in the code map.');

  lines.push('', '## Captured service contracts', '');
  const apiDir = path.join(targetDir, 'docs', 'architecture', 'api');
  const captured = fs.existsSync(apiDir) ? walkFiles(apiDir, '') : [];
  if (captured.length) {
    for (const f of captured) lines.push(`- \`docs/architecture/api/${f}\``);
  } else {
    lines.push(
      'Nothing captured yet — `docs/architecture/api/` holds each service\'s served contract, written at a bet\'s validation phase.'
    );
  }
  return lines.join('\n') + '\n';
}

async function generateViewsCommand() {
  const p = getPaths();
  banner();
  const engine = loadRepoMapEngine();
  if (!engine) { process.exitCode = 1; return; }

  // Reuse the cached map when it exists; build one when it does not, so the verb
  // stands alone at Setup Graduation.
  const mapPath = path.join(p.targetCacheDir, 'repo-map.json');
  let map;
  if (fs.existsSync(mapPath)) {
    map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
    c.dim(`  code map: .groundwork/cache/repo-map.json (${map.stats.files} files)`);
  } else {
    const built = await engine.generate({ cwd: p.targetDir, cacheDir: p.targetCacheDir });
    map = engine.write({ cacheDir: p.targetCacheDir, map: built.map, cache: built.cache });
    c.dim(`  code map: built ${map.stats.files} file(s) — no cached map existed`);
  }

  const langsByPartition = languagesByPartition(engine, map);
  const compose = readComposeServices(p.targetDir);
  const frontmatter = generatedViewsFrontmatter(headSha(p.targetDir));
  const outDir = path.join(p.targetDir, GENERATED_VIEWS_DIR);
  fs.mkdirSync(outDir, { recursive: true });

  const bodies = {
    'services.md': renderServicesView(map, langsByPartition, compose),
    'modules.md': renderModulesView(engine, map, langsByPartition),
    'contracts.md': renderContractsView(p.targetDir, map),
  };
  for (const name of GENERATED_VIEWS_FILES) {
    fs.writeFileSync(path.join(outDir, name), frontmatter + bodies[name]);
    c.ok(`Wrote ${GENERATED_VIEWS_DIR}/${name}`);
  }
  if (!compose) c.dim(`  no docker-compose file — services derive from the code map's partitions`);
  reportUnmapped(map.unmapped);
  console.log('');
}

// Advisory used by `check`: the generated views are stale when code moved after
// the commit they were generated from. Doc-only commits never make them stale.
function reportGeneratedViewsStatus(p) {
  const outDir = path.join(p.targetDir, GENERATED_VIEWS_DIR);
  ...
  for (const name of GENERATED_VIEWS_FILES) {
    ...
    if (fm && fm.generated_at_commit) stamps.add(String(fm.generated_at_commit));
  }
  if (!stamps.size) return;
  const head = headSha(p.targetDir);
  if (!head) return;
  const stale = [...stamps].some((sha) => {
    if (sha === head || sha === 'unknown') return false;
    try {
      const changed = execFileSync('git', ['diff', '--name-only', `${sha}..HEAD`], {
        cwd: p.targetDir, encoding: 'utf8',
      }).trim();
      // Doc and cache churn never invalidates a view derived from code.
      return changed.split('\n').some((f) => f && !f.startsWith('docs/') && !f.startsWith('.groundwork/'));
    } catch {
      return false; // an unreachable stamp (shallow clone, rewritten history) is not a drift claim
    }
  });
  if (stale) {
    c.warn(`Generated architecture views are stale — code changed since they were generated.`);
    console.warn(`         Refresh with \x1b[36m${GENERATE_VIEWS_COMMAND}\x1b[0m\n`);
  } else {
    c.ok(`Generated architecture views (${GENERATED_VIEWS_DIR}/) are current with HEAD.`);
  }
}
...
switch (command) {
  ...
  case 'generate-views':
    ...
    generateViewsCommand().catch((err) => {
      c.err(`generate-views failed: ${err.message}`);
      process.exit(1);
    });
    break;
  case 'gate':
