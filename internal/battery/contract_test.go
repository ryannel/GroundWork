...
// F117: three sentences of section 5 described the version before them. Each is
// now held to the line the code prints, on the re-check's own probes.
//
// A page is the one place a parsed shape is written down, so a sentence there
// that describes what a row used to do is worse than no sentence at all.

// The cut-message sentence. The row judges what it read and counts the message
// as cut, so a squash quoted inside the cap is still one squashed and one cut —
// which is what the page now says, in the row's own words.
func TestThePageSaysWhatACutMessageDoesToTheVerdict(t *testing.T) {
	dir := newRepo(t)
	writeSource(t, dir, "alpha/one.go", "package alpha\n")
	...
	if res.Outcome != Red {
		t.Fatalf("a squash inside a cut message came out %s: %s", res.Outcome, res.Evidence)
	}

	const said = "1 squashed, 1 cut"
	if !strings.Contains(res.Evidence, said) {
		t.Fatalf("the row said %q, and the page claims %q", res.Evidence, said)
	}
	if !strings.Contains(contractPage(t), said) {
		t.Errorf("the row prints %q on a cut message holding a squash, and the page never writes it", said)
	}
}

// The cluster rule. The page names the trailer key set, read the same way
// TrailerKeys' own pin reads it, so a reader can work out why one shape is
// green and the other red without running anything.
func TestThePageNamesTheTrailerKeysTheClusterReadUses(t *testing.T) {
	page := contractPage(t)

	for _, key := range board.TrailerKeys() {
		if !strings.Contains(page, "`"+key+"`") {
			t.Errorf("the cluster read counts %q, and the page never names it", key)
		}
	}

	// And the count, so a key added to the code without a sentence fails here.
	said := fmt.Sprintf("there are three of them: `%s`", board.TrailerKeys()[0])
	if len(board.TrailerKeys()) != 3 || !strings.Contains(page, said) {
		t.Errorf("the page says there are three trailer keys and the code holds %d",
			len(board.TrailerKeys()))
	}

	// The two shapes the page teaches a reader to tell apart. The page's own
	// word for each is read out of the sentence that teaches it, so a page that
	// taught the wrong one fails here rather than misleading a reader.
	for _, c := range []struct {
		...
		{
			"a prose label over a quoted line",
			"A `Notes:` label above a sentence starting `Slice:` is one trailer-shaped line, not two, " +
				"because `Notes` is not a trailer key",
			"Notes:\nSlice: demo_s1 was the one that broke.",
		},
		{
			"two trailer keys together",
			"`Bet:` and `Slice:` on adjacent lines are two, whatever the prose above them says",
			"The commit we are correcting carried\nBet: demo_bet\nSlice: demo_s1",
		},
	} {
		t.Run(c.name, func(t *testing.T) {
			taught, ok := verdictAfter(page, c.teaches)
			if !ok {
				t.Fatalf("the page does not teach %q with a verdict after it", c.teaches)
			}

			dir := newRepo(t)
			writeSource(t, dir, "docs/notes.md", "# Notes\n")
			commitAll(t, dir, "Record the review\n\n"+c.message+"\n")

			if got := runRow(t, dir, "history").Outcome; string(got) != taught {
				t.Fatalf("%s came out %s, and the page teaches %s", c.name, got, taught)
			}
		})
	}

	// And the warning the page owes whoever writes a commit message.
	for _, said := range []string{"never clears", "indentation or inside a fence"} {
		if !strings.Contains(page, said) {
			t.Errorf("the page does not warn a writer: it never says %q", said)
		}
	}
}

// The counter's counts sentence, held to the head word for word. The re-check
// found it listing counts the head no longer prints, in the order it no longer
// prints them.
func TestThePageListsTheCountsTheCounterPrints(t *testing.T) {
	page := contractPage(t)

	// The sentence, not the page. Every one of these words appears somewhere in
	// section 5, so a list that dropped one would pass a read of the whole page.
	const opens = "The `waiver-count` row's counts are"
	sentence, ok := sentenceFrom(page, opens)
	if !ok {
		t.Fatalf("the page holds no sentence opening %q", opens)
	}

	head := counterReport{}.head()
	for _, said := range []string{
		"waiver file", "grant", "merge", "rename", "at a limit", "misstated",
	} {
		if !strings.Contains(head, said) {
			t.Fatalf("the head is %q, and this test looks for %q", head, said)
		}
		if !strings.Contains(sentence, said) {
			t.Errorf("the head prints %q, and the page's list of counts never says it: %q", said, sentence)
		}
	}

	// The one count the head dropped, and where the page says it lives now.
	if strings.Contains(head, "unread") {
		t.Fatalf("the head is %q, and the page says the unread count is not in it", head)
	}
	if !strings.Contains(page, "The count of files that are not waivers is not among them") {
		t.Error("the page does not say where the count of files that are not waivers lives now")
	}
}

// verdictAfter reads the word the page teaches for one shape: the sentence that
// teaches it ends in an em dash and the row's own outcome word.
func verdictAfter(page, teaches string) (string, bool) {
	at := strings.Index(page, teaches)
	if at < 0 {
		return "", false
	}

	rest := page[at+len(teaches):]
	stop := strings.IndexAny(rest, ".\n")
	if stop < 0 {
		return "", false
	}

	_, said, found := strings.Cut(rest[:stop], "—")
	if !found {
		return "", false
	}

	said = strings.TrimSpace(said)
	if !slices.Contains(Outcomes(), Outcome(said)) {
		return "", false
	}
...
// sentenceFrom returns the sentence the page opens with some words, so a pin can
// hold a list to itself rather than to every word on the page.
func sentenceFrom(page, opens string) (string, bool) {
	at := strings.Index(page, opens)
	if at < 0 {
		return "", false
	}
