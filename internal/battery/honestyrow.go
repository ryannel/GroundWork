...
// helper from another file, an assertion library, a harness that keeps the
// test's own *testing.T — is read as honest. A shape below fires only when the
// scan can see the whole path and that path holds nothing that can fail.
//
// A test can fail in more ways than a t.Error, so more than that counts. A
// panic is a failure the runner reports, and so is anything a Must-named call
// does when it does not get what it must have. Both count as honest ways to
// fail.
//
// Known limits, written down rather than discovered later:
//
//   - Any method named Error, Fatal or Fail counts, whatever it hangs off. A
//     test whose only Error call is on a fake recorder reads as honest here.
//     That is a false negative this bet accepts: telling the test's own handle
//     from a look-alike needs the receiver tracked through assignments, and the
//     assertion-library shapes make that a careful change of its own.
//   - Only this file is followed. A helper in another file of the same package
//     is unresolvable, and an unresolvable helper reads as honest.
//   - A test whose assertions all sit behind a condition no reader would call
//     vacuous — one this scan cannot judge — is left alone.
...
func checkHonesty(c Context) Result {
	...
	for _, surface := range s.m.Surfaces {
		...
		cancel()
		if err != nil {
			// A surface that will not open does not cancel what the scan
	...
	case len(blocked) > 0:
		return Result{
			Outcome: Unrunnable,
			Evidence: cut(fmt.Sprintf("the honesty scan reads Go test source, and %s%s",
...
// is.
//
// The order matters. A test that skips itself unconditionally never reaches
...
func vacuousShape(file *ast.File, fn *ast.FuncDecl, src []byte, fset *token.FileSet) string {
	...
	if a.skipsItself(fn) {
		return "is skipped unconditionally, so it never runs"
	}
...
// skipsItself reports whether a test skips itself no matter what.
//
// Any statement of the test's own top-level list counts, not only the first
// one: t.Parallel() followed by t.Skip() is the common shape, and reading only
// the first statement would miss every one of them. What is not read is
// anything nested — a skip inside an if is a test that runs when the condition
// is false, and a helper that may skip is a helper. Reading either as a dead
// test would be the false red this row cannot afford.
func (a *assertions) skipsItself(fn *ast.FuncDecl) bool {
	for _, stmt := range fn.Body.List {
		...
		if ok && a.handles[recv.Name] && slices.Contains(skipMethods, sel.Sel.Name) {
			return true
		}
	}
...
func (a *assertions) walk(node ast.Node, vacuous bool) {
	ast.Inspect(node, func(n ast.Node) bool {
		...
		case *ast.CompositeLit:
			// A harness built around the test's own handle can fail the test
			// from anywhere later on. h := newHarness(t) is the shape, and the
			// scan cannot follow what h does next.
			a.escapes(n)
...
// escapes marks the test unknown when the testing handle is handed to
// something the scan does not follow: stored in a struct, assigned to a
// variable, returned out of a helper.
//
// Calls are not looked into here, because a call is judged on its own: a
// resolvable helper is walked, and an unresolvable one already marks the test
// unknown through its arguments. Descending into them would make every
// ordinary got := compute(t) unknowable and empty the row of its catches.
func (a *assertions) escapes(expr ast.Expr) {
...
func (a *assertions) call(call *ast.CallExpr, vacuous bool) {
	...
	case *ast.SelectorExpr:
		if _, ok := fun.X.(*ast.Ident); ok {
			name := fun.Sel.Name
			switch {
			case slices.Contains(failureMethods, name), mustName(name):
				a.fails(vacuous)
	...
	case *ast.Ident:
		// A panic fails the test the runner is running, and a Must-named call
		// panics when it does not get what it must have. Both are honest ways
		// for a test to fail, so both count.
		if fun.Name == "panic" || mustName(fun.Name) {
			a.fails(vacuous)
...
// fails records one way this test can fail.
func (a *assertions) fails(vacuous bool) {
	a.total++
	if vacuous {
		a.vacuous++
	}
...
// mustName reports whether a name promises to fail when it does not get what
// it asked for: MustParse, MustCompile, and every helper written in their
// image.
func mustName(name string) bool {
	rest, found := strings.CutPrefix(name, "Must")
	if !found {
		return false
	}
	if rest == "" {
