...
Copy `groundwork-v2-0.3.0.tgz` into the app's `vendor/` directory, then run there:

```sh
npm install --save-dev ./vendor/groundwork-v2-0.3.0.tgz
npx --no-install groundwork-v2 init --name "My app"
npx --no-install groundwork-v2 serve
```
