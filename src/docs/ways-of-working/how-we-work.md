...
**Brownfield** projects — an existing codebase — run a parallel track, implemented, not a roadmap item: `groundwork-scan` classifies the repo, maps it deterministically, and commits an orientation page; `groundwork-ops-adopt` bolts on the missing operational layer and registers the surfaces without regenerating the app; `groundwork-architecture-extract` records the boundaries and budgets in force; and `groundwork-infra-adopt` documents the services and consolidates the maturity roadmap. The product-brief and design-system extracts run on demand rather than up front. There is no MVP phase — the first bet cold-starts its own discovery from that roadmap. Both tracks converge on the same `docs/` set and the same Delivery Loop below.
