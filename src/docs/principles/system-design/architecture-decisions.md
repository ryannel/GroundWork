...
In an agent-led codebase, the record set is not just human documentation — it is the decision-context layer an agent reads before proposing or revisiting a choice. The most recent records carry the most relevant constraints, so we keep them lean, linked, and assumption-explicit: that is what lets a human or an agent make a *consistent* next decision instead of contradicting one made last quarter. Records are written to be machine-consumable for the same reason every other interface is ([Agent-Native Systems](../ai-native/agent-native-systems.md)).

### 9. Govern by advice, and pair the record with a check

A record is not a gate. "Governed" means an **advice process** — the decider seeks advice from those affected and those with expertise but keeps the decision — not a central review board teams route around. And where a decision is mechanically checkable, we pair it with a **fitness function** that fails the build on violation: the record documents the choice, the fitness function assures it still holds ([Evolutionary Architecture](evolutionary-architecture.md)).
...
- [Hexagonal Architecture](hexagonal-architecture.md) — the boundary decisions most often worth recording.
...
- *MADR* ([adr.github.io](https://adr.github.io/)) — the Markdown ADR template and its status lifecycle; the source of "3–5 elements are the essence."
- *Decision governance* (the 2025–26 shift from documenting decisions to governing them) — the ownership/assumption/review-trigger framing.
