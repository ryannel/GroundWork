...
GroundWork ships runners for these mediums today: `playwright` (web), `flutter-integration` (Flutter, via the app's own `integration_test` harness), and `playwright-electron` (Electron). A graphical surface on any other platform — a native iOS/SwiftUI app, an Android app, a desktop-native shell — needs a check built to the contract below, registered under a new test medium the generator recognises.
...
2. **Navigation — no dead ends.** Every screen the surface reaches has a way back or onward; no flow strands the user with no exit. The check drives the real navigation between the surface's screens and confirms each is reachable and leavable.
