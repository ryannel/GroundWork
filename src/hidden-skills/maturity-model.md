...
Every service boundary exposes a pinned, machine-readable contract — OpenAPI, AsyncAPI, or proto — and `docs/architecture/api/*.md` transcribes from it.
...
**Signal:** every service named in `docs/architecture/index.md` has a contract file referenced from its `docs/architecture/api/<service>.md`.
...
**Signal:** the stance appears under Binding Constraints in `docs/architecture/index.md`; the contract-conformance system tests report no breaking drift against the promoted specs in `docs/architecture/api/`. Below two independently deployed surfaces this dimension assesses `n/a` — it activates the moment the second one registers.
