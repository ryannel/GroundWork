...
2. **Pass the design for review.** The design lives as a directory of section files; **invoke the review subagent** (Protocol 9) with `document_path: docs/bets/<bet-slug>/technical-design/` (the directory itself) and `document_type: technical-design`. The reviewer walks the directory and reads each section file individually — its `READ` manifest names what it covered, which a pre-concatenated blob would hide. The gate is fail-closed (Protocol 8): proceed only on a parseable `VERDICT: PRESENT`; a review that errors, hangs, or returns no verdict follows Protocol 9's failure path — and a manifest whose `skipped` bucket contains any section file is not a covering verdict: re-invoke rather than proceed on it.
3. **Revise loop.** If the verdict is **REVISE**, apply every 🔴 Critical finding directly to the technical design — rewrite only the affected section file(s) under `docs/bets/<bet-slug>/technical-design/` rather than producing a list of suggestions — then run the review again on the same directory path. Two 🔴 findings are product choices, not structural defects, and the closed list stops there: **Pitched capability undesigned** and **Silent scope growth**. A fix that keeps the pitch's commitments intact — restoring the dropped capability, trimming the grown scope back — clears the gate the same way any other 🔴 fix does, and is additionally recorded with `npx groundwork-method findings add --bet <bet-slug> --bucket patch --title "<the finding>"` followed by `findings disposition --bet <bet-slug> --id <id> --as fixed`, so it surfaces in the sign-off ledger below instead of vanishing into the diff. A fix that would change what the bet delivers instead — cutting a pitched capability, keeping the grown scope — is scope-changing: stop and ask the owner before re-invoking the review, because that choice is theirs to make, not the agent's to make for them. Every other 🔴 finding follows the revise loop as normal. The revise cap is a hard stop, not a target to push past: after 3 REVISE verdicts, stop, surface remaining 🔴 findings as 🟡 Advisory, and disclose that the review did not reach **PRESENT** (Protocol 8).
4. **Triage advisories before presenting.** When the verdict is PRESENT, triage every 🟡 Advisory before the Transition walk — never after a full read. A cheap, uncontested completeness or wording fix is applied immediately; disclose these in one summary line at the walk's opening. A finding that encodes a real choice — a trade-off the user should rule on, not the agent — is steer-worthy: leave it unapplied, record it with `npx groundwork-method findings add --bet <bet-slug> --bucket decision-needed --title "<the finding>"` so it joins the sign-off ledger below, and attach it to the section of the walk it belongs to. When in doubt, treat it as steer-worthy. Design-phase 🟡s surface at this sign-off, where they are cheap to act on while the design is still open — not held for the Decomposition review. For any advisory the review withheld for length (its topics-named summary line), record each named topic the same way so it lands in committed engine state instead of vanishing, and disclose the count at the walk's opening.
...
Once the technical design has passed review, present it to the user as the design contract for this bet. The decision trail behind it lives in engine state, shaped through drafting so the pending queue stays truthful: a decision the user already answered live — an architecture call in Step 1.9, the POC choice in Step 1.92, a UX-pattern pick in Step 1.95 — is recorded with `npx groundwork-method decisions add --bet <bet-slug> --question "<q>" --default "<their answer>" --rationale "<one line>"` immediately followed by `decisions ratify --bet <bet-slug> --id <id> --response "<their verbatim answer>"`, never left pending; an applied reviewer 🔴 or a steer-worthy 🟡 from Step 2.5 is recorded there, per that step's mechanism — the 🔴 disposed `fixed`, the 🟡 left open until the walk below rules on it — and disposed the moment it does (`findings disposition --bet <bet-slug> --id <id> --as fixed|dismissed-with-reason|deferred-with-owner`, with the `--note` the disposition demands). Nothing recorded for display sits open past this sign-off — in the pending queue or the findings ledger; an entry left open here reads as unfinished work to every later engine check.
...
Its sign-off runs per Protocol 12 (`operating-contract.md`): the host's native review surface, where one exists, carries the approval pass on `technical-design/` at its canonical path. Open with the ledger itself, at two densities. Items the user personally ruled on — the decisions above, already answered live — recap in one line each: the question and their answer; re-walking them at full depth would re-spend attention already paid. Everything they have not yet seen walks at full density, with alternatives considered rather than just what was verified: the steer-worthy advisories from Step 2.5's triage, each attached to the section of the walk it belongs to below; any applied reviewer 🔴; and — independent of what the agent chose to self-report — a caller-side class that is always full: a contract delta in `03-api-design.md` (an endpoint shape, an error case), a one-way-door or irreversible choice, and anything the pitch's Stakes names as blast radius. A choice the agent made confidently and the reviewer PRESENTed is exactly the one nothing else would surface. Everything the PRESENT verdict covered mechanically compresses to one line per section naming what was verified and by which check, plus the canonical path where it renders on the docsite, expandable on request.

The section-by-section walk stays the teaching layer regardless of density — it builds the user's grasp of the contract, not just their approval of it. Orient with the pitch's topology graph (the system the bet plays in). For a `graphical-ui` surface, before walking **UI Design**, run `groundwork design build --bet <slug>` and put `.groundwork/cache/visual/token-sheet.html` in front of the user — rendered inline where the host offers that surface (Protocol 12), by path where it does not — so they sign the rendered treatment rather than the token string. They react in the conversation, naming a card by its printed id; apply what they ask for to `.groundwork/config/brand-tokens.json`, rebuild, and show it again. The sheet is generated from that file and the repo's own components, never hand-authored, so what they sign is what the build compiles; `groundwork design check` names any value that traces to no token, and those are not the user's decisions to approve. Skip entirely for a `cli`- or `agentic-protocol`-only bet. Then walk the **UI Design** (`technical-design/01-ui-design.md`) subsection by subsection — the wireframes and states, where the user's mental model of the bet lives — then the **Data Flows & Business Logic** (`technical-design/02-data-flows.md`): how the system works; then the **API Design** (`technical-design/03-api-design.md`): the interfaces; then the **Schema & Data Design** (`technical-design/04-data-design.md`): how data is stored. When the user wants to push a section deeper — or a section reads thin against the quality standard above — load `.groundwork/skills/groundwork-elicit/instructions.md` and follow it. A comment the user leaves on the design document is applied, re-flowed across the affected sections for cohesion, and re-gated against Protocol 8/9 before the design is re-presented leading with what changed, per Protocol 12's delta rule — never the full document again.
