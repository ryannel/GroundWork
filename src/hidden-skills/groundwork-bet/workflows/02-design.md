...
3. **Revise loop.** If the verdict is **REVISE**, apply every 🔴 Critical finding directly to the technical design — rewrite only the affected section file(s) under `docs/bets/<bet-slug>/technical-design/` rather than producing a list of suggestions — then run the review again on the same directory path. The revise cap is a hard stop, not a target to push past: after 3 REVISE verdicts, stop, surface remaining 🔴 findings as 🟡 Advisory, and disclose that the review did not reach **PRESENT** (Protocol 8).
...
Once the technical design has passed review, present it to the user as the design contract for this bet, its sign-off run per Protocol 12 (`operating-contract.md`): the host's native review surface, where one exists, carries the approval pass on `technical-design/` at its canonical path. The section-by-section walk below stays as the teaching layer regardless — it builds the user's grasp of the contract, not just their approval of it. Orient with the pitch's topology graph (the system the bet plays in), then walk the **UI Design** (`technical-design/01-ui-design.md`) subsection by subsection — the wireframes and states, where the user's mental model of the bet lives — then the **Data Flows & Business Logic** (`technical-design/02-data-flows.md`): how the system works; then the **API Design** (`technical-design/03-api-design.md`): the interfaces; then the **Schema & Data Design** (`technical-design/04-data-design.md`): how data is stored. When the user wants to push a section deeper — or a section reads thin against the quality standard above — load `.groundwork/skills/groundwork-elicit/instructions.md` and follow it. A comment the user leaves on the design document is applied, re-flowed across the affected sections for cohesion, and re-gated against Protocol 8/9 before the design is re-presented — Protocol 12's comment-resolution loop.
