...
|---|---|---|
| `00-overview.md` | The orientation a reader gets first: the bet's business logic and key data flows in prose, with mermaid diagrams (a topology `graph` of the services/components this bet touches, and a `sequenceDiagram` per non-trivial cross-service/data flow). Absorbs the **Data Flows** narrative and frames the whole bet. | `technical-design/00-overview.md` |
| `01-surface-design.md` | The **Surface Design** section — one subsection per surface, with the micro-polish spec for `graphical-ui` surfaces. | `technical-design/01-surface-design.md` |
| `02-capability-design.md` | The **Capability Design** prose: API Contracts (purpose, error guidance, rationale) and the Data Schema narrative. | `technical-design/02-capability-design.md` |
...
The technical design covers the **entire bet** — not per-milestone. Write it before any decomposition into milestones or slices. The section order is the design's logic: the **overview** orients the reader in the business logic and data flows first; **Surface Design** is drafted next, because the contract must serve the experiences — never the other way around; **Capability Design** (API contracts, data schema) is then written once beneath all of them, surface-neutral and headless.

**Overview (`00-overview.md`):** Talk through the bet's business logic and its key data flows in prose — the most valuable file in the directory. Require fenced ` ```mermaid ` blocks (these render on GitHub and the Fumadocs site): a topology `graph` of the services and components this bet touches is **required**, and a `sequenceDiagram` is **required** for each non-trivial cross-service or data flow. For each significant data path, describe what triggers it, which services handle it, what persists, and the key design decisions that shaped it. Skip trivial CRUD; focus on paths where timing, service boundaries, or failure modes are non-obvious.

**Surface Design (`01-surface-design.md`):** One subsection per surface in the pitch's `surfaces:` frontmatter. Each subsection describes what that surface's users observe and interact with, organised by view, command, or interaction — not by feature or service. For each: the purpose, its states (loading, active, empty, error, degraded), and its key interactions. For `graphical-ui` surfaces, each subsection also carries the **micro-polish spec** from Step 1.95 — the motion, atmosphere, and static-micro tokens the surface resolves to — so craft is specified, not left to the build to improvise. Write each subsection in the vocabulary of that surface's interface type, from its design track in `docs/design-system.md`: screens and states for `graphical-ui`, commands and output for `cli`, request/response turns for `agentic-protocol` — a bet spanning a web app and a CLI carries one subsection in each vocabulary. Each subsection is what that surface's milestone tests will assert against — it must be specific enough to make a test pass or fail unambiguously. When the project has no surface registry (`docs/surfaces.md` absent), the product has a single implicit surface: write one subsection for it in the project's interface medium and skip every other surface consideration in this workflow — the result is today's section with the headings one level deeper. A single-surface registry likewise produces exactly one subsection, with no added questions.

**Capability Design (`02-capability-design.md`)** is the headless core of the bet — everything in it must be designable, implementable, and provable with no surface running. Its two subsections (the data flows that frame the contract live in `00-overview.md`):
...
Once the technical design has passed review, present it to the user as the design contract for this bet. Open with the overview (`technical-design/00-overview.md`) — its diagrams and data-flow narrative orient the user in the bet — then walk the Surface Design (`technical-design/01-surface-design.md`) subsection by subsection, where the user's mental model of the bet lives, then the Capability Design (`technical-design/02-capability-design.md`): API contracts and schema. When the user wants to push a section deeper — or a section reads thin against the quality standard above — load `.groundwork/skills/groundwork-elicit/instructions.md` and follow it.
