...
3. **Tear down the isolation.** Remove the worktree (`git worktree remove <path>`, then `git worktree prune` — never `rm -rf`, which strands `.git/worktrees/` metadata) and delete the merged branch — locally, and, if it was pushed for backup during delivery (`04-delivery.md`, "Push the bet branch as you go"), its remote too (`git push origin --delete bet/<bet-slug>`). The `bet/<bet-slug>/approved` tag stays — it is the bet's permanent seal in history.
