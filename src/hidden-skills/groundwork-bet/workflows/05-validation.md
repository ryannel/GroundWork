...
Execute the full bet-progress test suite: `./dev test bet <bet-slug>` (or `pytest tests/bets/<bet-slug>/` directly). Every test must pass before advancing — and run the **prose-integrity reconciliation once over the whole bet**: `npx groundwork-method seal verify --bet <bet-slug>`. The `bet/<bet-slug>/approved` tag is the sealed baseline — amendments re-point it as they land — so a clean exit confirms the decomposition and technical-design prose still matches what the owner approved, and every built test still proves what its slice's Proof-of-work prose describes. (The per-slice `seal verify` already guarded each step during delivery; this is the whole-bet confirmation.) The contract is the approved prose; the tests and implementation were built this phase and are supposed to have changed. A non-zero exit means the suite drifted from the approved prose without a recorded amendment — not what the user approved; flag it and revert. `npx groundwork-method state --bet <bet-slug> --check` runs this and the findings-closure and pack-freshness gates as one aggregate exit — the engine-side precondition for everything below.
...
1. **Tier 1 — the deterministic floor is green.** Confirm the three system gates passed in Step 2's run — `test_render_smoke.py`, `test_a11y_smoke.py`, `test_token_conformance.py`, defined at `04-delivery.md` Milestone close (`workflows/delivery/step-03-milestone-close.md`). A red layer or a platform with no runnable check blocks the bet — fail-closed, not skipped.
2. **Tier 2 — spec-conformance inspection happened.** Confirm each graphical milestone recorded its per-screen spec-conformance verdict during delivery (the `Visual:` line in the closing slice's delivery commit, from `workflows/delivery/step-03-milestone-close.md`). That verdict is the designer-judged check: does the rendered surface match the written micro-polish spec, including the dimensions computation cannot assert — optical alignment, restraint, and whether the composition reads as considered. A milestone that closed with no verdict did not run the inspection — route it back.
3. **The whole-bet experience judgment.** Each milestone proved its own piece; now judge the assembled product end to end, dispatching the experience-auditor lens (`briefs/experience-auditor.md`, the designer persona). It looks for cross-milestone gaps a per-milestone review structurally cannot see — a flow that works within each milestone but breaks at the join. **Dedup the re-drive** so this pass adds coverage rather than repeating it: drive the **milestone seams** in full, and re-drive a **whole milestone only when it took commits after its own milestone-close experience audit** (a milestone already audited and unchanged since is already covered — re-driving it whole buys nothing). A no here — a dead end, a half-built pattern, a screen that works but is not usable — **fails the bet**; route it back to delivery to fix, do not wave it through to `delivered`.

   **Read the pitch's Stakes to set the review load, never the floor.** The pitch's per-bet **Stakes** field (`templates/pitch.md` — blast radius, reversibility, how much a human must hold to vouch for it) shapes how much of this end-to-end judgment the owner walks with the auditor: a one-way-door, wide-blast-radius bet earns a heavier owner walk-through; an iterate-behind-a-flag change earns a lighter one. This is the per-bet shaping the owner already authored and nothing yet read — **not the rejected project-level stakes axis**, and it conditions nothing in the floor: the deterministic tiers, the per-slice lenses, the milestone honesty audit, the front-door proofs, and this experience gate all run in full regardless of Stakes. Stakes tunes owner attention, never rigor.
...
For each `docs/` artifact, scan the bet conversation and the delivered code for what now contradicts the document. Apply targeted updates. Report what changed, in the terms Protocol 11 sets (operating contract). **One rule covers all three persona adoptions below:** when a refinement is structural (architecture), a design change (design system), or vision-level (product brief), adopt the matching persona — architect, designer, or product (`.groundwork/skills/groundwork-architect/SKILL.md`, `groundwork-designer/SKILL.md`, `groundwork-product/SKILL.md`) — so the update carries the document's own reasoning standard; the persona governs that judgment only, never the voice the report above is delivered in.
...
Write `docs/bets/<bet-slug>/retrospective.md`: the patterns found, the follow-through audit results, any discovery alerts, the readiness state, and the action items — each with a stable ID (`<bet-slug>-R1`, `-R2`, …) so the next retrospective can audit them mechanically. The ID is a mechanical audit key, not something spoken in conversation — when an action item comes up in discussion, name it in plain language, never by its ID. Append the action items as bullets under `## Bets` in `.groundwork/cache/discovery-notes.md`, each carrying its ID — the next bet's Discovery phase already reads that section and cites the ID only in the pitch's frontmatter or links, never in the pitch body.
...
**By topology** (`workflows/delivery/topologies.md`): a **monorepo** merges one branch. For **submodules**, fast-forward each affected submodule's branch and push it, then land the final superrepo gitlink-bump merge last. For a **polyrepo**, fast-forward each repository's branch in producer-before-consumer order (or gated on the contract check), the change-set manifest recording the set; there is no single merge.
