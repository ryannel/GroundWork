...
**Open with the checkpoint walkthrough (whole-bet scope).** Before the owner drives the product, dispatch the checkpoint walkthrough (`briefs/checkpoint-walkthrough.md`, `frontier` tier, read-only — not a gate) over the **whole bet's** accumulated change: the change organized by concern, the highest-blast-radius spots tagged `[auth]/[schema]/[contract]/[data]/[infra]`, suggested front-door observations for the drive below, any default+veto decisions still pending ratification (`docs/bets/<bet-slug>/decisions.md`), and the deferred/maturity rows the bet touched. It keeps the owner's system comprehension current for the product they are about to own in production — it complements the design walk, never substitutes for it.
