...
**Name each milestone as the product outcome it reaches, in the consumer's own words** — the same test of "who sees it and what they see," compressed into a title: "Browse and search the library," not a coined codename, an internal component, or invented terminology. The milestone name is the one identifier the user carries across weeks of delivery — a name they have to decode is a thread they lose.
...
Present the milestone list as a fenced ` ```mermaid ` `flowchart TD` — one node per milestone carrying its plain-language outcome name, annotated with its consumer and the risk that rung retires, edges in build order — so the sequencing review is judged from a picture before it is judged from prose. Follow it with the **sequencing rationale** for each: what proof Milestone 1 provides, why Milestone 2 can only follow it, and so on, and which consumer observes each. The review focuses on **ordering, whether each milestone names a real outcome a named consumer observes at their surface, and whether the ladder sums to a complete experience** — not implementation detail. Revise the ordering — and the diagram with it — until the user is satisfied before proceeding.
...
**A stated budget maps to a proof.** When the design system or the bet's `technical-design/` states a user-felt budget on a path this bet builds — a latency ceiling, a start-up bound, a frame budget — some milestone proof line or named fitness function must re-check it, quoting the number. A budget with neither is an orphan: a POC measured it once and nothing ever measures it again (`02-design.md` Step 1.92 records that exact failure recurring). This is the same decomposition-time discipline as the front-door rule above, applied to the design's numbers instead of its behaviour; the review blocks an orphan budget.
...
**A slice's identifier is `N.M` — milestone number, slice number — and no other scheme.** No letter bands, no `M.<n>` forms, no scheme invented for this bet — any second scheme collides with the milestone numbers it sits beside. `N.M` is unambiguous by construction: `2.3` can only mean milestone 2's third slice.
...
- **Ripple** *(optional)* — the shared symbol(s) this slice renames or reshapes (a rename, a signature widening, a moved type), when other code already depends on them. Omit for slices that only add. This is the signal Delivery's capsule assembly acts on: the driver runs reference analysis against committed code and hands the worker the caller list, so every consumer is named *before* the change instead of discovered by whichever targets happen to compile (`workflows/delivery/step-02-slice-loop.md`, §1).
...
| `decomposition/index.md` | The tree's landing page — the milestone ladder as a `mermaid flowchart TD` (Step 2) plus one demonstrable-goal line per rung, so a reader sees the whole plan before opening any folder. | `decomposition/ladder-index.md` |
...
**The full ladder, the first rung sliced.** Write every milestone's `index.md` now — the complete ladder of headline proofs the user approves — together with the tree's own `decomposition/index.md` landing page, whose ladder diagram and demonstrable-goal lines summarize the same rungs before a reader opens any folder. Write slice files only for the **first milestone**. A later milestone's folder holds its `index.md` with the headline proof and its slice list deferred (the `milestone-index.md` template's *authored on arrival* affordance) until Delivery opens it; its slice files are written then. This is *plan just enough* on disk: the whole ladder is visible and reviewable, but only the rung you are about to climb is detailed.
...
2. **Pass the tree for review.** The decomposition lives as a tree of files; **invoke the review subagent** (Protocol 9) with `document_path: docs/bets/<bet-slug>/decomposition/` (the directory itself) and `document_type: decomposition`. The reviewer walks the directory and reads each file individually — its `READ` manifest names what it covered, which a pre-concatenated blob would hide. The gate is fail-closed (Protocol 8): proceed only on a parseable `VERDICT: PRESENT`; a review that errors, hangs, or returns no verdict follows Protocol 9's failure path — and a manifest whose `skipped` bucket contains any decomposition file is not a covering verdict: re-invoke rather than proceed on it.
3. **Revise loop.** If the verdict is **REVISE**, apply every 🔴 Critical finding directly to the affected milestone `index.md` or slice file. Rewrite sections rather than annotating them, then run the review again on the same directory path. Two 🔴 findings are product choices, not sequencing defects, and the closed list stops there: **Design not covered** and **Scope beyond the design**. A fix that keeps the pitch's commitments intact — adding the missing milestone or slice, trimming the grown scope back to the design — clears the gate the same way any other 🔴 fix does, and is additionally recorded with `npx groundwork-method findings add --bet <bet-slug> --bucket patch --title "<the finding>"` followed by `findings disposition --bet <bet-slug> --id <id> --as fixed`, so it surfaces in the sign-off ledger below instead of vanishing into the diff. A fix that would change what the bet delivers instead — cutting a milestone or capability the design covered, keeping the grown scope — is scope-changing: stop and ask the owner before re-invoking the review, because that choice is theirs to make, not the agent's to make for them. Every other 🔴 finding follows the revise loop as normal. The revise cap is a hard stop, not a target to push past: after 3 REVISE verdicts, stop, surface remaining 🔴 findings as 🟡 Advisory, and disclose that the review did not reach **PRESENT** (Protocol 8).
4. **Triage advisories before presenting.** When the verdict is PRESENT, triage every 🟡 Advisory before the Proof of Work walk — never after a full read. A cheap, uncontested completeness or wording fix is applied immediately; disclose these in one summary line at the walk's opening. A finding that encodes a real choice — a trade-off the user should rule on, not the agent — is steer-worthy: leave it unapplied, record it with `npx groundwork-method findings add --bet <bet-slug> --bucket decision-needed --title "<the finding>"` so it joins the sign-off ledger below, and attach it to the milestone or slice it belongs to. When in doubt, treat it as steer-worthy. For any advisory the review withheld for length (its topics-named summary line), record each named topic the same way so it lands in committed engine state instead of vanishing, and disclose the count at the walk's opening.
...
First render the proof board: `npx groundwork-method proofs --bet <bet-slug> --write` writes `docs/bets/<bet-slug>/proofs.md` — every agreed front-door case as one row (milestone, consumer, the case verbatim, its proof), derived whole from the tree. The board is the one page the user tracks from this sign-off to the final drive; it is regenerated, never hand-edited, and carries no approval state — the approval is the tag below, on the prose.

The decision trail behind it lives in engine state, shaped through Step 2's sequencing review and Step 6's revise loop so the pending queue stays truthful: a sequencing or scoping decision the user already answered live is recorded with `npx groundwork-method decisions add --bet <bet-slug> --question "<q>" --default "<their answer>" --rationale "<one line>"` (add `--milestone <N>` when the decision is scoped to one) immediately followed by `decisions ratify --bet <bet-slug> --id <id> --response "<their verbatim answer>"`, never left pending; an applied reviewer 🔴 or a steer-worthy 🟡 from Step 6 is recorded there, per that step's mechanism — the 🔴 disposed `fixed`, the 🟡 left open until the walk below rules on it — and disposed the moment it does (`findings disposition --bet <bet-slug> --id <id> --as fixed|dismissed-with-reason|deferred-with-owner`, with the `--note` the disposition demands). Nothing recorded for display sits open past this sign-off — in the pending queue or the findings ledger; an entry left open here reads as unfinished work to every later engine check.

Read the pitch's Stakes field before pacing what follows: a one-way-door, wide-blast-radius bet walks every section below at full density; an iterate-behind-a-flag bet takes the two-density default this section describes. Either way the review gate above, and every mechanical check inside it, already ran in full — Stakes tunes owner attention, never rigor.

The sign-off runs per Protocol 12 (`operating-contract.md`): the host's native review surface, where one exists, carries the approval pass on the decomposition tree at its canonical path, comments and all — but the walkthrough below stays the teaching layer either way; the sign-off moves to the reviewed surface, the walk does not. Open with the ledger itself, at two densities. Items the user personally ruled on — the decisions above, already answered live — recap in one line each: the question and their answer; re-walking them at full depth would re-spend attention already paid. Everything they have not yet seen walks at full density, with alternatives considered rather than just what was verified: the steer-worthy advisories from Step 6's triage, each attached to the milestone or slice it belongs to; any applied reviewer 🔴; and — independent of what the agent chose to self-report — a caller-side class that is always full: a contract delta in `technical-design/03-api-design.md`, a one-way-door or irreversible sequencing choice, and anything the pitch's Stakes names as blast radius.

Then walk the milestone map — ordering rationale, who observes each milestone, demonstrable goals, judged against Step 2's `mermaid` ladder diagram — and the agreed cases **row by row on the proof board**: one traversal, the board rows as the spine, the tree as drill-down and the comment surface. For each row, what it proves, where that traces in the design, and why it is the right proof. The proof is prose, but the scrutiny is assertion-grade — the user is approving the agreed front-door test cases that become the definition of done, so pace this walkthrough like the design decision it is (Protocol 4), not a confirmation formality. The first milestone's slice files present as one scan table instead — id, capability gist, proof gist, owner service — with an invitation to open any file on request: the process already treats slice breakdown as free to steer, needing no formal record, so a slice-by-slice walk at the milestones' assertion grade would spend attention where none is needed.

Where the user challenges a proof, fix the prose and continue; a comment left on the reviewed document instead is applied, re-flowed across the affected milestones and slices for cohesion, and re-gated against Protocol 8/9 before the tree is re-presented leading with what changed, per Protocol 12's delta rule — never the full document again.

On approval, **commit and tag the decomposition as the recorded baseline**: commit `docs/bets/<bet-slug>/decomposition/` (the full milestone ladder plus the first milestone's slices) together with the finalized `technical-design/` (e.g. `bet(<bet-slug>): approve decomposition`) — and, under git, tag that commit `bet/<bet-slug>/approved`. That commit and tag are the user's signature on the agreed front-door cases — the integrity anchor the rest of the bet keeps honest. The readiness gate (`groundwork-review/checklists/implementation-readiness.md`) blocks delivery without the tag; it is the baseline that delivery's prose-integrity reconciliation holds the prose to.

The anchor leaves a trail, and the trail is lightweight. **Steering how slices break down is free and needs no record** — adjusting the path to a milestone as delivery teaches you is the plan working as intended. **Changing what a milestone proves** — editing or dropping an agreed front-door case — is an owner-approved move recorded beside the prose: amend the affected `index.md` or slice file, commit it with a message that says what changed and why (`bet(<bet-slug>): amend milestone <N> proof — <reason>`), and re-point the `bet/<bet-slug>/approved` tag to that commit, so a later context (a resumed delivery, a validator) always finds the tag on the currently-approved state. Authoring a later milestone's slices on arrival, and adding a new rung when a postmortem reveals the ladder is missing one, are the same kind of recorded, additive event, each re-pointing the tag in turn. Nothing is broken or reset — the record is the commit history of the decomposition tree, the tag always names its head, and Delivery's prose-integrity check reconciles each built test against the current approved prose it traces to. (This holds whether or not the project is under git: the standing rule is that every built test still proves what its slice's Proof-of-work prose describes, and a change to that prose is a recorded amendment, not a quiet edit.)
