...
5. Present the reviewed pitch to the user for sign-off, per Protocol 12 (`operating-contract.md`): the host's native review surface, where one exists, runs the approval pass on the draft itself; the walkthrough of the pitch elements above is the teaching layer alongside it, not the only way to read the draft. A comment the user leaves on the pitch is applied, re-flowed for cohesion across the affected elements, and re-gated against Protocol 8/9 before the pitch is re-presented — Protocol 12's comment-resolution loop, not a separate procedure here. On explicit approval, promote `.groundwork/cache/bet-pitch-draft.md` to `docs/bets/<bet-slug>/pitch.md` by moving the file (the `move_file` tool, or `mv` via the shell) — do not read the draft and rewrite its contents.

   **This promotion is also where the bet's branch and worktree open** — the moment the pitch reaches its canonical `docs/` path, on the bet branch, that Protocol 12 points at. Every artifact from here forward — this pitch, the technical design, the decomposition — is authored on `bet/<bet-slug>`, in its own worktree, not on trunk: the bet's evolution is visible in one place from day one (the docsite's in-flight view reads the bet branch), and Delivery no longer discovers its isolation late. Open them now, following the branch/worktree mechanics in `04-delivery.md`'s Git workflow section — the canonical home for those mechanics, not restated here. Write `pitch.md` inside that worktree.
