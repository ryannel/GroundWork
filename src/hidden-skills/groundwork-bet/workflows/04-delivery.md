...
**The isolation exists before the red board.** The bet arrived here already on `bet/<bet-slug>`, in its own worktree — opened at pitch promotion (`01-discovery.md`), carried through design and decomposition. Step 0.5 commits the red board into that existing isolation.

**Legacy reconcile: a bet promoted before this rule.** If the bet is not already on its own branch and worktree from its earlier phases, open them now: branch `bet/<bet-slug>` from a clean `origin/<trunk>`, in a worktree under a gitignored path. One branch lives in one worktree — never check the same branch out twice. This is the catch-up path, not the default expectation.
...
**Hard stop — pause and ask now — only for a decision that is destructive, scope-changing, or proof-weakening.** Everything else takes the **default+veto** path: record the recommended default with `npx groundwork-method decisions add --bet <bet-slug> --milestone <N> --question "<q>" --default "<recommended>" --rationale "<one line>"` (the queue is committed engine state at `.groundwork/bets/<bet-slug>/decisions.json`), proceed on it, and batch it to the next natural pause (a milestone postmortem, the validation prelude) for ratification. A defaulted decision that needs a commit rides a single `[provisional]` amendment; **the `bet/<bet-slug>/approved` tag moves only on the owner's ratification** — `npx groundwork-method decisions ratify --bet <bet-slug> (--id <id> | --all) --response "<the owner's verbatim words>"`, which stamps their response as durable state — never on a provisional default. This converts the class of stalls where a whole session blocks on one binary question into minutes.
...
Each step is a separate file under `delivery/`. **Read one step file fully, execute it, and load the next only at that file's transition line** — never load two steps at once. The `board.yaml: step` field is the pointer to the current step; the table below maps it to a file. A fresh context resumes by loading `.groundwork/skills/groundwork-persona/instructions.md` — the chat posture a resumed session would otherwise never re-load — then running `npx groundwork-method state --bet <bet-slug>` — the composed engine view (seal integrity, open findings, pending decisions, pack freshness, the board's step pointer) in one read — plus `memlog.md` for the narrative, then **reconciling against `./dev bet status` and the git log of delivery commits** — the board is a convenience, tests and git are the truth, so on any divergence the git/suite state wins and the board is rewritten to match. With no board (or a corrupt one), resume the old way: the first red slice is where to pick up, and a milestone whose headline stub is red but with no slice files yet is the next one to open.
