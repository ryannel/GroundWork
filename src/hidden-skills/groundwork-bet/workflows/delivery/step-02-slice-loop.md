...
- **Prose integrity — one mechanical check.** `npx groundwork-method seal verify --bet <bet-slug>`: exit 0 means the sealed prose (decomposition + technical-design) still matches the `bet/<bet-slug>/approved` tag — an amendment re-points the tag, so a ratified change reads as sealed. A non-zero exit lists prose that drifted from the tag *without* a recorded amendment — a proof, acceptance criterion, or API shape weakened, dropped, or loosened — which is a `decision-needed` finding. Most slices change no prose, so this is usually a no-op — do not narrate it into a ritual.
- **Honest green — the gaming tells, plus two conditional checks.** The implementation must satisfy the proof for the right reason, against the real product; the gaming tells are canonical in `briefs/acceptance-auditor.md` (the milestone honesty audit) and apply here, run inline by the driver as `decision-needed` findings. Three further checks fire only where they can bite, not on every slice:
  - **On a UI-touching slice** (one whose diff touches UI source — components, styles, screens): run `npx groundwork-method tokens scan --bet <bet-slug>` — it flags raw color/font/spacing/motion literals that bypass the design-token set; its leads triage through the same buckets below, defaulting to `patch` when a token set exists.
...
- **Coverage auditor** (`briefs/coverage-auditor.md`) — the diff, the slice's Required Capabilities, and the stack's testing strategy; judges the permanent tests rolled out against it: error/boundary cases at the rigour of the happy path, a unit test for complex logic, a trace assertion on an observable path, named states for a `graphical-ui` slice. A sociable test that executes a branch without asserting on it is a gap even on a green board. Once the slice is committed, `npx groundwork-method mutate --bet <bet-slug> --slice <test-file> --since <prior-slice-sha> -- <test command>` runs the deletion test mechanically (it refuses a dirty working tree, never stashing uncommitted work); exit 1 — still green with the implementation reverted — is a hard `patch` finding, not advice. Default-on; skip only with a mechanical no-obligation reason (the slice rolls out no permanent test the strategy asks for), stated in one line.

For a slice that touches a UI surface, also run the mechanical companion alongside the lenses — `npx groundwork-method wiring scan --bet <bet-slug>` flags controls built but never wired (empty or TODO-only handler bodies, handler-shaped functions with no reachable caller) — and triage its leads through the same buckets below, fail-closed: an unwired control on a UI slice is a `patch` finding, not a note.
...
Update the working state (`../04-delivery.md`, *Working state* — it never gates): rewrite `board.yaml` to mark this slice `done` with its commit sha and advance `step`, append one memlog line (`./dev bet log <bet-slug> -- "slice <N.M> closed (<sha>): <Notes gist>"`), mirror the worker's ~20-line report to `.groundwork/cache/bets/<bet-slug>/reports/<slice-key>.md` (so the postmortem and validation re-read it without holding it live), rebuild this milestone's context pack so the next worker inherits the slice's learnings (`npx groundwork-method pack build --bet <bet-slug> --milestone <NN>` re-harvests the ledger, reports, and memlog; put the slice's `Notes` gist in the driver-notes block if it is judgment the harvest cannot carry), and refresh the bet's status page (`npx groundwork-method status --bet <bet-slug> --write`) so the docsite reflects this close. Carry any amendment through to the memlog too.
