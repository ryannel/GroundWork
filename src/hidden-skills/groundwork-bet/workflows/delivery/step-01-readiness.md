...
When every 🔴 item passes, state so in one line, update `docs/bets/<bet-slug>/pitch.md` frontmatter to `status: delivery`, and tell the user what this transition means: the design is locked and approved, so from here the work is building it — fewer questions land on them, decisions arrive batched, and pauses come only at the granularity they chose in Step 0.7. This report and the granularity choice below are both spoken across the boundary Protocol 11 governs (operating contract). Write the active-lane sentinel — `printf '%s\n' '<bet-slug>' > .groundwork/cache/active-lane` — so the capture reminder hook stays silent while this lane drives edits; Validation removes it at bet close.
