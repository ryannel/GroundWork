...
Before any slice work, verify the bet is actually executable. First run `npx groundwork-method gate readiness --bet <bet-slug>` — it computes the mechanical 🔴 items fail-closed: the document chain exists, the pitch is at `status: delivery`, the decomposition tree is structurally complete (`meta.json`, every milestone `index.md`, the first milestone sliced, every slice link and `meta.json` page resolving), each milestone and first-milestone slice names a Proof of work and a `Test file:`, and the `bet/<bet-slug>/approved` tag is present. A non-zero exit blocks delivery and names each failed check. Then judge the items the gate cannot compute, against `.groundwork/skills/groundwork-review/checklists/implementation-readiness.md`: the *Unreviewed artifact* 🔴 (the decomposition/design review reached `VERDICT: PRESENT`), *Slice without a contract* 🔴 (each slice's API/data shapes are present in the prose design), and the 🟡 currency items (discovery-note contradictions, stale upstream, maturity blockers). If the checklist file is absent, stop and report it; do not improvise from memory. This is the delivery-side counterpart of Decomposition Step 6's review: that gate judged the prose's authorship against `groundwork-review/checklists/decomposition.md`; this one only confirms the same artifacts still exist, are current, and agree with each other now.
...
**Compile milestone 1's context pack.** Run `npx groundwork-method pack build --bet <bet-slug> --milestone 1` — it writes `.groundwork/cache/bets/<bet-slug>/milestone-01-context.md` (shape: `../../templates/milestone-context.md`): the mechanical pointers and learnings harvested from the sealed state, stamped `compiled_from` = the approved-tag sha. Then fill the pack's driver-notes block with your judgment — the engineer Context-Routing rows this milestone's workers should load and the worktree environment facts; the block survives every rebuild verbatim. The pack carries pointers and learnings, never contract text; it is what the Slice Loop's dispatch capsule points each worker at.
...
Consult `.groundwork/skills/groundwork-bet/references/bet-progress-tests.md` for the placeholder pattern and quality criteria. Run the suite once and confirm **every stub is red** — because the implementation does not exist, not an import or fixture error. Confirm every milestone stub carries an active front-door assertion in the consumer's medium, not a commented-out one — `groundwork honesty scan --bet <bet-slug>` flags a stub whose interface proof is commented out. Commit the red board (e.g. `bet(<bet-slug>): materialize red board`) before opening the first slice — it is the build artifact the slice loop fills in, free to change. Then refresh the proof board — `npx groundwork-method proofs --bet <bet-slug> --write` — and tell the user in one line that the board page now shows each agreed case beside its red stub; no action is asked of them.
