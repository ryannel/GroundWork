...
- **A UI slice's proof renders at realistic content scale, not fixture scale.** A recurring class of owner-reported layout bugs shipped on green suites because tests asserted logic against tiny fixtures while a full-resolution image pushed every control off-screen. Where the slice delivers UI, its proof (or its permanent tests) exercises the standard realistic-scale fixtures — an oversized image (~4000px intrinsic), fully-populated rows or grids, the longest realistic strings — and asserts the chrome stays in bounds and controls remain hittable, not merely present in the tree. Presence-based checks pass on broken layouts; geometry assertions bite.
