...
- [ ] 🔴 **Approval tag absent**: under git, the `bet/<slug>/approved` tag does not exist — the approved decomposition tree and technical design were never sealed, so the delivery prose-integrity reconciliation has no baseline to hold the prose to — nothing would stop the approved scope weakening unnoticed. An unsealed contract is a draft, not a definition of done. (Not under git: confirm the bet record names the approval baseline the reconciliation falls back to.)
