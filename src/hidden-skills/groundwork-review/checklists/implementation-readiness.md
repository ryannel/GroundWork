...
- [ ] 🟡 **Test-review surface stale**: `docs/bets/<slug>/test-review.md` is missing an entry for a test in the suite, or describes a test that no longer exists or whose prose proof no longer matches the test — the user approved something other than what delivery will execute against.
