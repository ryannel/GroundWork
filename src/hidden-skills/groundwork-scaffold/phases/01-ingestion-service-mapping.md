...
- **What stays bet/MVP work:** prompt caching a large shared context, streaming responses, structured outputs, a moderation/safety gate, and tool use. Record these in the scaffold hand-off as work the first bet must build, and say so plainly when presenting the scaffold. The moderation/safety gate gets one extra hook: when the product brief names user-facing input, Phase 5 records its deferral as a maturity roadmap row — deferred is legitimate, invisible is not.
...
**Offer the documentation site (one decision, default include).** Independent of the architecture's services, offer the user the browsable site for the project's own `docs/` — and frame it as what it actually is under the review protocols: the standing place where everything they will be asked to approve or ratify renders — the product brief, the architecture, each bet's pitch, design, decomposition, and live status page — reachable any time via `./dev docs`, and never part of the running product. Present it once as a single decision with a real veto (Protocol 4), **defaulting to including it** — the review workflow assumes a render surface exists, and a veto here means every future approval reads as raw markdown. If they accept (or don't object), add `docs-site --name docs` to the execution plan — it registers as a native `./dev` runner and shows up in `infrastructure.md`'s footprint matrix like any other managed unit (Phase 5). If they veto, add nothing and say nothing further. The docs site is a developer affordance, never inferred from the architecture and never a service the architecture must justify.
