...
5. **Reconcile capability footprints** — for every port in `.groundwork/capability-ports.json`, confirm its footprint is real, not just declared: a `compose-service` provider has a running container; a `runner` provider appears in `./dev status` (probe `./dev status --json` and check its `runners` array); an `env` provider has its variables present in the service's config / `.env.example`; a `none` raw gateway has its strict-xfail contract test in the service's suite (the bet is visible, not forgotten). The managed set `./dev` reports must equal the architecture's surfaces + services + runners — a mismatch is a gap between what the architecture declared and what the scaffold built, and is resolved here, not papered over in the doc.

Do not advance to Phase 5 until the entire system boots cleanly, all tests pass, and every capability footprint reconciles.
