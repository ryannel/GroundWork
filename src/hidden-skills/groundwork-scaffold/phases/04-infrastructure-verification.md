...
4b. **Prove the shipped gates fire** — the scaffold ships fitness functions (depguard in `.golangci.yml`, import-linter in `pyproject.toml`, the a11y smoke in the system tests), and a gate that has never failed is an assumption, not a gate. For one service per stack present: introduce a deliberate inward-dependency violation (an outer-layer import inside the core), run `./dev lint`, confirm it fails naming the boundary rule, and revert the probe. Where a graphical surface exists, confirm the a11y smoke actually ran in step 3's suite. Then run `./dev audit` once so the dependency and secret gates execute against the fresh tree. Record the probe results — gate name, red-then-reverted — in `scaffold-cache.md`; Phase 5's maturity assessment cites them as evidence.
