...
2. Promote the design system to `docs/design-system.md` with a move operation (do not re-emit the body through the model).
3. **Promote brand tokens.** Move `.groundwork/cache/brand-tokens-draft.json` to `.groundwork/config/brand-tokens.json`. Verify it validates against the contract and carries the correct tier for the interface type. This file is persistent config — it is never deleted at cache cleanup, and infra adoption depends on it.
...
9. Confirm completion, recommend a fresh context, and immediately load and execute `groundwork-orchestrator`. Do not ask the user to invoke it.
