...
4. **Tear down the scan cache (this phase owns it — unless a deferred extract still needs it).** Check `state.json`'s `phase_states` first: when any extract phase is `deferred`, skip the scan-cache deletion entirely — `.groundwork/cache/scan/` and `scan-state.json` are that phase's inputs, and the last deferred extract to commit deletes them (the orchestrator's Deferred Phases route carries this obligation). With no extract deferred, delete `.groundwork/cache/scan/` (overview and any remaining findings), `.groundwork/cache/scan-state.json`, and the consumed architecture-extract hand-off. **Preserve `.groundwork/cache/repo-map.json`** in every case — it is a first-class artifact `groundwork-check` and the bet loop reuse for impact analysis, regenerable on demand by `npx groundwork-method repo-map`. Delete `docker-compose.yml.bak` only after confirming the merged compose boots; otherwise leave it for the user.
