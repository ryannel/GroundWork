...
`ways_of_working` catches process machinery that lives *inside* a partition — a service-local test convention, a package-local scaffolder, a partition-scoped contributor guide. Fill it only when Stage 1 classified the repo `"methodology": "incumbent"`; on every other repo it stays empty and its findings file is never created. `groundwork-methodology-adopt` regenerates its own inventory when it runs, so what lands here is a head start it reads if the file survives, never the input it depends on.
