...
**Preferred path — a semantic code-intelligence tool.** If an LSP-backed code-intelligence tool is available in this environment (GroundWork registers **Serena** as an MCP server at init), use it to read the symbol structure and import/export relationships exactly rather than inferring them from prose. Walk the tree with `get_symbols_overview` per package, resolve the most-referenced symbols with `find_symbol`, and trace edges with `find_referencing_symbols` — LSP-accurate, free of the hallucinated edges an LLM produces. Cache the result to `.groundwork/cache/repo-map.json`: the module/partition boundaries, the import/call edges, a centrality ranking of the most-referenced symbols, and the contract/symbol index. Serena answers per-symbol rather than emitting a whole-repo graph, so assemble `repo-map.json` from these queries and LLM-infer only the relationships it cannot enumerate. Find the tool with a tool search for the code-intelligence or symbol capability before assuming it is absent.

**Fallback path — LLM inference.** When no code-intelligence tool is available (e.g. a sandboxed or headless environment without the Serena MCP server, or without `uv`), infer the same structure from targeted reads — entry points, manifests, and import statements — and write `repo-map.json` in the **same shape**. The downstream contract is identical; only the means of producing it differs. Do not let the fallback change what the file holds.
