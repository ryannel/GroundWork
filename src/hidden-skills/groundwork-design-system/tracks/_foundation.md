...
4. **Present the rendered token sheet — the seam where prose approval becomes visual approval.** The Phase 5b clusters approved the `visual` block's values in prose; before that approval is final, render them.

   Run `groundwork design build`. It generates a self-contained `.groundwork/cache/visual/token-sheet.html` from the `visual` block just written: the semantic palette as swatches in both light and dark themes, type roles at their real size and line-height, elevation and surface treatments, and motion profiles with their durations and easings. **Do not hand-author this file.** It is generated precisely so that what the user approves and what the app compiles are the same values — a sheet written by hand looks equally convincing, proves nothing, and drifts on the next run.
