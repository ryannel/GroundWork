...
This is the brownfield brief phase, and it runs on demand rather than in the default track — the scan already committed the orientation shape of this document, so you are deepening a page that exists rather than starting from nothing. You recover the product from evidence, fill the irreducible gaps through a short focused conversation, and commit. The output is indistinguishable from a greenfield brief — every downstream phase reads it the same way.
...
The shared operating contract at `.groundwork/skills/operating-contract.md` (contract v1) governs how this skill operates. Read it before taking any other action. This is a Sequential Setup phase: it follows the full cache, hand-off, Downstream Context, review, and pacing protocols. It consumes the scan baseline under the Protocol 7 brownfield exception — it may read `scan/product-findings.md`, `scan/overview.md`, `scan-state.json`, and `repo-map.json`, and no other phase's cache. Open the phase — and precede every question that blocks on the user — with the Setup Progress Header (operating contract, Sequential Setup).

This phase is deferrable, and under the default brownfield track it does not run at all: scan-lite drafts and commits the orientation shape of `docs/product-brief.md` and records this phase `collapsed`. You run when the user asks for the full brief, or when a bet's discovery needs depth the orientation page does not carry — which means the scan findings this skill was written around are usually absent. Stage 1 branches on that.
...
Running after Setup Graduation lands here by definition — the context store is gone and the orientation page stands. That is the expected entry, not a contract failure: Adopt/Upgrade is the right mode, and the post-graduation carve-outs (no Downstream Context file, no hand-off file) come from the operating contract's Sequential Setup section.
...
Build your model of the product before speaking. This is silent.

Read, in order:

1. **`.groundwork/cache/scan/product-findings.md`** — the scan's distilled product signals: value proposition, user-facing capabilities, product surface, inferred users, monetisation. **When this file does not exist, run the scoped read pass below instead** — it is the default case, not an error.
2. **`.groundwork/cache/scan/overview.md`** when present — repo shape and the interaction medium(s), which fixes the product's surface. Otherwise take both from `.groundwork/cache/scan-state.json`'s classification and confirmed `surfaces` array.
...
4. In Adopt/Upgrade mode, the existing `docs/product-brief.md` — your primary source. The orientation page scan-lite committed enters here the same way: it is a real prior document, and this phase deepens it rather than replacing it.

### When no findings slice exists — regenerate your own inputs

The default brownfield track never runs the whole-repo digest pass, so the findings slice this phase was built around is usually absent. Generate the equivalent yourself, scoped to what a brief actually needs.

Read `.groundwork/cache/repo-map.json` and let its centrality ranking choose your targets: the user-facing entry points, the route and command surfaces, the auth and permission model, and the manifests. Read those; skim nothing else. The digest schema at `.groundwork/skills/groundwork-scan/references/digest-schema.md` is your read discipline — its `purpose`, `product_signals`, `inferred_users`, `licensing_signals`, and `interface_type` fields name exactly what you are looking for and what to stop looking for.
...
**The honest cost note:** deferring this phase moved the read cost here; it did not remove it. The saving is that only the partitions a product brief depends on get read, and only when someone actually needs the brief.
...
The recovered brief must read like it was written by someone who understands the product — not a description of the code. "A FastAPI service with three routers" is code description. "A booking system that lets venue managers hold inventory across channels without double-selling" is a product brief. The committed document is the lean template shape — Purpose & Problem, Audience in two-line entries, Surfaces, Non-goals & Hard Rules — and the depth bar applies to the **understanding and its carriers**: each user type is a mental model (travels in the hand-off), each capability conveys what it does and why it matters (the inventory travels in the hand-off — the architecture extract frames services from it), the experience walks the macro journey (hand-off). If the synthesis reads like the scan findings with formatting, no extraction work was done — the contribution is the translation from mechanism to meaning.
...
4. Write the hand-off file to `.groundwork/cache/handoff/product-brief-extract.md` from `.groundwork/skills/templates/handoff.md`. Under the lean brief shape this file is **load-bearing, not overflow**: it carries the recovered capability inventory (what each does for the user, why it matters — the architecture extract frames services from this), the macro experience journey per surface, and each user type's mental model — then the classic overflow: rejected framings, deferred decisions with their reopening trigger, unformalised design or architecture instincts. Omit empty sections (Protocol 6).
5. **Delete the consumed findings slice** when one existed: remove `.groundwork/cache/scan/product-findings.md`. Leave `scan/overview.md`, `scan-state.json`, and `repo-map.json` in place; later phases still read them. When this phase runs after Setup Graduation and it was the last deferred phase reading a preserved scan cache, delete the whole `.groundwork/cache/scan/` tree and `scan-state.json` — the teardown `groundwork-infra-adopt` skipped on its behalf. Preserve `repo-map.json` always.
...
9. **Clear the register entry.** When `state.json`'s `phase_states` carries an entry for `product-brief-extract` — `collapsed` from scan-lite, or `deferred` from the user's choice — remove it in this same step. The artifact now carries the full shape, and a stale entry would keep a finished phase looking unsettled.

10. Confirm completion, recommend a fresh context, and immediately load and execute the `groundwork-orchestrator` skill. Do not ask the user to invoke it — it routes on, whether that is the next setup phase or back to the work that needed this brief. Record nothing else in `state.json`: the orchestrator reconciles this phase's completion from its committed artifacts.
