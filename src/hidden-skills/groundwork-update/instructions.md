...
ordering dependencies (graduate ADRs before nesting the architecture docs; rename
`docs/ux-design.md` before seeding the design system's anchors into it), so they are
...
| **Product brief shape** (`product-brief-shape`) | `.groundwork/skills/groundwork-product-brief/product-brief-template.md` — the section list both the greenfield and the extract writer draft against | `docs/product-brief.md` carrying a retired section heading, at any heading level: `System Purpose`, `The Problem`, `Target Users`, `Capabilities`, `The Experience`, `Domain Constraints`, `Out of Scope`, or `Success Indicators`. Two or more of `Target Users`, `Capabilities`, `The Experience` together confirm it — a lone `Out of Scope` in an otherwise current brief is a user's own heading, not a legacy shape. | **Distil to shape.** Pauses on a product judgment: which success indicators survive into the merged opening paragraph, and what the capability prose becomes once `docs/surfaces.md` owns capability state. |
| **Design system shape** (`design-system-shape`) | Two canonical shapes, both current: the greenfield spec's Draft Layout (`.groundwork/skills/groundwork-design-system/tracks/_foundation.md`) and the brownfield decision-record-over-code shape (`.groundwork/skills/groundwork-design-system-extract/instructions.md` Stage 4) | `docs/design-system.md` carrying no heading spelled exactly `Commitments` (at any level), or no `## Pattern Index` heading. A present `docs/ux-design.md` is the same signal one rename earlier — the Naming family moves that file to `docs/design-system.md`, so detect on it here too and run this family **after** Naming, or the renamed file leaves the run still missing its anchors. | **Seed anchors, keep content.** Which of the two canonical shapes the project already holds is not this family's call — it seeds the anchors into whichever one is there. |
...
worker's relocate / rename / graduate / restructure / distil / seed-anchors / re-promote
recipes.
