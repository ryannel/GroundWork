...
to build and reuse `.groundwork/cache/repo-map.json` and to run impact analysis. Serena
answers per-symbol rather than emitting a whole-repo graph, so assemble the map from these
queries and LLM-infer only what it cannot enumerate.
