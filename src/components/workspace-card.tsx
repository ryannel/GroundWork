...
import { relTime, useQuery, type WorkspaceSummary } from '@/data/store'
...
export function WorkspaceCard({ s }: { s: WorkspaceSummary }) {
  ...
  const productNames = `${products.length} ${products.length === 1 ? 'product' : 'products'}: ${products.map(p => p.name).join(', ')}`
  return <Link to={`/w/${w.slug}`} className="workspace-destination" style={hueStyle(w.hue)}>
    <div className="workspace-card-top">
      <WorkspaceEmblem workspace={w} products={products} />
      <span className="workspace-card-open" aria-hidden="true"><ArrowUpRight size={18} /></span>
    </div>
    <h3>{w.name}</h3>
    ...
    <footer>
      <div className="workspace-activity">
        ...
        <span><strong>{ideas}</strong> {ideas === 1 ? 'idea' : 'ideas'}</span>
      </div>
      {s.lastActivity && <time dateTime={s.lastActivity} title={`Latest feature update: ${new Date(s.lastActivity).toLocaleString()}`}>
        Updated {relTime(s.lastActivity)}
      </time>}
    </footer>
  </Link>
}
