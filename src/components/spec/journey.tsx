  ...
  return <div>
    ...
    {selected ? <div className="journey-explorer">
      ...
      <article className="journey-detail" key={selected.id}>
        ...
        {mockup ? <div className="journey-preview"><MockupCard m={mockup} /></div> : <p className="rounded-md border border-dashed border-border p-4">No screen is linked to this step yet.</p>}
        <div className="eyebrow mb-3">Behind this experience</div>
        {selected.flow?.length ? <Link className="inline-flex items-center gap-2 text-small text-accent mb-4" to={`/f/${featureId}/flow?trace=journey:${selected.id}`}>Trace this step through the system <ArrowRight size={14} /></Link> : <p className="text-small text-fg-muted">No system flow linked yet.</p>}
        <RefRow groups={[
        ...
        {params.has('step') && <button className="text-small mt-4" onClick={() => setParams(p => { const n = new URLSearchParams(p); n.delete('step'); return n })}>Reset selection</button>}
      </article>
