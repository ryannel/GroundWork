...
import { CircleDashed, Diamond, FileCheck2, Hammer, Lightbulb, CheckCheck, type LucideIcon } from 'lucide-react'
...
export function FeatureRow({ feature, showProduct = true, showWorkspace = false, context: contextOverride, className }: {
  feature: Feature
  showProduct?: boolean
  ...
  return <Link to={`/f/${feature.id}`} className={cn('feature-work-row', className)}>
    <span className="feature-work-title">
      ...
      <span><strong>{feature.title}</strong>{context && <small>{context}</small>}</span>
    </span>
 ...
 */
export function FeatureWorkList<Id extends string>({ label, views, view, onView, summary, titleColumn, rows, renderRow, empty }: {
