...
export function featureInProduct(feature: Feature, productId: string, components: Component[]): boolean {
  return productId === 'all' || feature.productId === productId
    || feature.touches.some(id => components.some(component => component.id === id && component.productId === productId))
}
...
export function connectedProductIds(feature: Feature, components: Component[]): string[] {
  const productIds = feature.touches.map(id => components.find(component => component.id === id)?.productId)
  return [...new Set(productIds.filter((id): id is string => !!id && id !== feature.productId))]
}
