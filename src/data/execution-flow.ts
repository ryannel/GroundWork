...
export function executionFlowIssues(component: Component): string[] {
  ...
  const active = { endpoint: component.api?.endpoints, schema: component.api?.schemas, data: component.data?.records, message: component.messaging?.messages, job: component.jobs, flow: component.executionFlows }
  unique((component.retiredObservations ?? []).map(item => `${item.kind}/${item.id}`), 'retired observation IDs')
  for (const item of component.retiredObservations ?? []) if (active[item.kind]?.some(entry => entry.id === item.id)) issues.push(`Retired identity is still active: ${item.kind}/${item.id}`)
  ...
  for (const flow of component.executionFlows ?? []) {
    ...
    if (flow.trigger?.kind === 'message' && !component.messaging?.messages.some(message => message.id === (flow.trigger as { messageId: string }).messageId && message.direction === 'inbound')) fail('Message trigger requires an inbound message in this component')
    ...
    for (const step of flow.steps) {
      ...
      for (const name of step.unresolvedDependencyNames ?? []) if (!component.unresolvedDependencies?.some(dependency => dependency.name === name)) fail(`${step.id}: unknown unresolved dependency ${name}`)
    }
