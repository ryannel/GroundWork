...
const navigationParams = new Set<string>(['apiEntity', 'dataEntity', 'messagesEntity', 'jobsEntity', 'schema', 'finding', 'flow'])
/**
...
export function catalogLocationFor(target: CatalogTarget): CatalogLocation {
  switch (target.kind) {
    ...
    case 'finding': return { finding: target.id }
...
export function catalogTargetOf(location: CatalogLocation): { kind: CatalogKind; id: string } | undefined {
  if (location.finding) return { kind: 'finding', id: location.finding }
