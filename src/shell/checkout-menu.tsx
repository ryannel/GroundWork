...
export function CheckoutMenu() {
  ...
  return <details className="checkout-menu" ref={menu} onKeyDown={event => {
    ...
    <summary aria-label="Checkout details"><GitBranch size={14} /><span>{context.ref ?? context.branch ?? (context.isGit ? 'Detached' : 'Local')}</span>{!context.editable && <small>Read-only</small>}{!connected && <small>Offline</small>}<ChevronDown size={12} /></summary>
    <div className="checkout-panel">
      <p className="eyebrow">{mode === 'central' ? 'Via Groundwork Hub' : 'Standalone viewer'}</p>
      <label>Plan version<select value={context.ref ?? ''} onChange={event => { window.location.href = branchHref(event.target.value || undefined) }}><option value="">{context.branch ?? (context.isGit ? 'Detached HEAD' : 'No Git repository')} · working files</option>{activity.branches.map(branch => <option key={branch} value={branch}>{branch} · committed</option>)}</select></label>
