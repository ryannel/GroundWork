...
| `docs/bets/<slug>/test-review.md` (approved plain-language proofs) + the approval commit recorded as `approval_commit` in `decomposition.json` | 03-decomposition Step 6.6 + approval (commit); amendments rewrite the entry and re-commit | 04-delivery Step 4 test-integrity reconciliation; implementation-readiness gate; 05-validation |
