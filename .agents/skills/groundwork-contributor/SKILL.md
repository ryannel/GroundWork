...
| `docs/api/<service>/` canonical specs (captured code-first from the running service) | 05-validation capture (snapshot of each touched service's served contract) | the generated contract-conformance system tests (run by the project's `./dev test`); groundwork-check |
