...
| Methodology-twin convergence (one way of working at the end; incumbent canon ingested, in-flight work unfractured) | `methodology_twin` (suite-owned fixture) | Bounded runs when the scan-classify signal or the convergence corpus changes; full run before a release |
