...
// F117: the help text reads the close scope rather than writing it out. R14
// says later bets add their rows to that scope, and a hand-written list would
// go stale the first time one did — with the help promising one thing and the
// refusal doing another.
func TestTheHelpTextNamesTheRowsTheCloseRefusalNames(t *testing.T) {
	newRepo(t)

	_, _, errOut := call(t, "verify", "--lst")
	if !strings.Contains(errOut, "--close") {
		t.Fatalf("the usage text does not describe --close: %s", errOut)
	}

	// Every row the scope holds, in the scope's own order, as one phrase. A
	// help text that named them in another order, or named a row the scope
	// dropped, would be a second list.
	said := strings.Join(battery.CloseScope(), ", ")
	if !strings.Contains(errOut, said) {
		t.Errorf("the usage text does not name the close scope %q: %s", said, errOut)
	}

	// And the refusal names the same rows, from the same place.
	var refused bytes.Buffer
	refuseClose(&refused, "groundwork verify", battery.RunResult{})
	if !strings.Contains(refused.String(), said) {
		t.Errorf("the refusal names %q and the help text names something else", refused.String())
	}
}
