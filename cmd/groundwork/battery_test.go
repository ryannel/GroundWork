...
func TestVerifyPrintsTheWholeSummary(t *testing.T) {
	...
	const want = "9 rows: green 9, red 0, waived 0, quarantined 0, unrunnable 0"
...
func TestVerifyRedPrintsTheWholeSummary(t *testing.T) {
	...
	// printed, never a silent skip and never green. Two rows are green. The plan
	// row, because a repo with no docs/plan has no plan to misstate. The chain
	// row, because the run journals each row as it finishes, so by the time the
	// chain row runs the ref holds this run's own lines — and they are chained.
	const want = "9 rows: green 2, red 2, waived 0, quarantined 0, unrunnable 5"
	if !strings.Contains(out, want+"\n") {
		t.Fatalf("the summary line is not %q:\n%s", want, out)
	}
}
