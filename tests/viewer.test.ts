...
test('instruction refresh installs project startup scripts without replacing user scripts', async t => {
  ...
  assert.equal(pkg.scripts['plans:standalone'], 'groundwork serve')
  assert.equal(pkg.scripts['plans:hub'], 'groundwork hub')
  assert.equal(pkg.scripts.dev, 'vite')
})

test('instruction refresh moves projects from the groundwork-v2 CLI to groundwork', async t => {
  const root = await tempDir(t, 'groundwork-rename-')
  await writeFile(path.join(root, 'package.json'), JSON.stringify({ name: 'app', scripts: { 'plans:start': 'groundwork-v2 start', plans: 'groundwork-v2 serve --port 4400', 'plans:other': 'groundwork-v2x' } }))
  await writeFile(path.join(root, 'AGENTS.md'), '# Agents\n\n## Groundwork planning\n\nFor Groundwork planning, read [.groundwork/GUIDE.md](.groundwork/GUIDE.md). Use the installed `groundwork-v2` CLI or MCP server and preserve checkout context and revision checks.\n')
  await installInstructions(root)
  const pkg = JSON.parse(await readFile(path.join(root, 'package.json'), 'utf8'))
  assert.equal(pkg.scripts['plans:start'], 'groundwork start')
  assert.equal(pkg.scripts.plans, 'groundwork serve --port 4400')
  assert.equal(pkg.scripts['plans:other'], 'groundwork-v2x')
  const agents = await readFile(path.join(root, 'AGENTS.md'), 'utf8')
  assert.match(agents, /installed `groundwork` CLI/)
  assert.doesNotMatch(agents, /groundwork-v2/)
})
