...
import { ContentError, createRepository, loadContent } from '../shared/content.ts'
...
const rejects = (data: Record<string, unknown>, message: RegExp) => assert.throws(() => load(data),
  error => error instanceof ContentError && message.test(error.message))
...
test('new workspace, product and draft feature are discoverable without UI registration', () => {
  ...
  input['products/p-new.json'] = { id: 'p-new', workspaceId: 'w-new', slug: 'new-product', name: 'New product', kind: 'library',
    repositories: [{ repository: 'fixture/new', role: 'owned' }] }
  input['components/c-new.json'] = {
    id: 'c-new', repo: 'fixture/new', name: 'New component',
  ...
  input['features/f-new/feature.json'] = { id: 'f-new', productId: 'p-new', title: 'New plan', stage: 'idea', ownerId: 'ryan-nel',
    touches: ['c-new'], updatedAt: '2026-09-05T10:00:00Z' }
...
test('shape validation rejects unknown fields, stages, versions and malformed dates with a file path', () => {
  for (const [key, value] of [['stage', 'approved'], ['updatedAt', 'yesterday'], ['titel', 'Typo']]) {
    const input = copy();
    input['features/f-1/feature.json'][key] = value
...
test('a missing owner or product fails before a partial repository is created', () => {
  ...
  input['features/f-1/feature.json'].ownerId = 'missing';
    input['features/f-1/feature.json'].productId = 'missing'
...
test('duplicate IDs, ambiguous routes and orphaned sections are rejected', () => {
  ...
  duplicate['members/duplicate.json'] = duplicate['members/ryan-nel.json'];
    rejects(duplicate, /duplicate member/)
  const slug = copy();
  slug['products/p-cart.json'].slug = 'pricing';
    rejects(slug, /duplicate "pricing"/)
  const orphan = copy();
  orphan['features/f-orphan/purpose.json'] = { problem: 'Why', outcome: 'What' };
    rejects(orphan, /require feature.json/)
})
test('cross-product references work within a workspace; cross-workspace references fail', () => {
  const input = copy();
  input['features/f-1/feature.json'].touches.push('c-cart-svc');
    assert.doesNotThrow(() => load(input))
  input['features/f-1/feature.json'].touches.push('c-cli');
  rejects(input, /another workspace/)
})
test('structured references to declared components use the home repository identity', () => {
  const input = copy()
  const home = 'acme/catalog-home'
  input['components/postgres.json'] = { id: 'postgres', repo: home, name: 'Postgres' }
  input['products/p-cart.json'].repositories.push({ repository: home, role: 'owned' })
...
test('section links, decision outcomes and API boundaries must target the right objects', () => {
  const missing = copy();
  missing['features/f-2/tests.json'].cases[0].steps = ['missing'];
    rejects(missing, /unknown journey reference/)
  const decision = copy();
  decision['features/f-2/flow.json'].nodes.find((n: any) => n.id === 'region').logic.branches[0].edgeId = 'e-sdk-cart';
    rejects(decision, /branch must leave/)
  const boundary = copy();
  boundary['features/f-2/api.json'].contracts.find((c: any) => c.id === 'api-quote').from = 'c-sdk';
    rejects(boundary, /does not match the boundary/)
})
test('action boundaries cannot leave the action and visible nodes cannot overlap', () => {
  const input = copy();
  input['features/f-2/journey.json'].steps[0].flowEdges = ['e-region-quote'];
    rejects(input, /leaves this action/)
  const overlap = copy();
  const nodes = overlap['features/f-2/flow.json'].nodes;
    nodes[1].col = nodes[0].col;
    nodes[1].row = nodes[0].row;
    rejects(overlap, /flow positions/)
})
test('schema fields are unique within each parent, including nested objects', () => {
  const input = copy();
  const schema = input['features/f-2/api.json'].contracts.find((c: any) => c.id === 'api-quote').responseSchema.after
  schema.push(structuredClone(schema[0]));
  rejects(input, /responseSchema.after: duplicate/)
})
test('external design references reject unsafe protocols and live prototypes', () => {
  ...
  input['features/f-2/design.json'].mockups[0] = { id: 'd-cart', kind: 'image', title: 'Example', ref: 'javascript:alert(1)' };
  rejects(input, /http\(s\) URL/)
})
test('member names and viewer identity resolve centrally', () => {
  const input = copy();
  input['members/ryan-nel.json'].name = 'Renamed member'
  const { q } = createRepository(load(input))
...
test('a completely blank system needs no member or viewer record', () => {
  const snapshot = load({ 'project.json': {} })
...
test('chronological sorting uses actual timestamps even when offsets differ', () => {
  const input = copy();
  input['features/f-1/feature.json'].updatedAt = '2026-09-05T12:00:00+02:00';
    input['features/f-2/feature.json'].updatedAt = '2026-09-05T10:30:00Z'
...
test('component API guides validate their ownership, capability IDs, and contract links', () => {
  ...
  missing['features/f-2/api.json'].guides[0].capabilities[0].contractIds = ['unknown-contract'];
    rejects(missing, /unknown api reference/)
  const wrongOwner = structuredClone(input);
  wrongOwner['features/f-2/api.json'].guides[0].componentId = contract.from;
    rejects(wrongOwner, /another component's API/)
  const duplicate = structuredClone(input);
  duplicate['features/f-2/api.json'].guides.push(duplicate['features/f-2/api.json'].guides[0]);
    rejects(duplicate, /duplicate/i)
})
