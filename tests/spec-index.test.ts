...
test('indexes accept IDs that look like Object.prototype members', () => {
  ...
  assert.deepEqual(new Set(ix.testNodes['toString' as string]), new Set(['toLocaleString', 'propertyIsEnumerable']))
  assert.deepEqual(ix.testEdges.toString, ['hasOwnProperty'])
  assert.deepEqual(new Set(ix.testSpans['toString' as string]), new Set(['client', 'server']))
