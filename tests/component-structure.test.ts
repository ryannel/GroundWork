...
} from '../shared/component-structure.ts'
import { buildIndex, lensFor } from '../shared/spec-index.ts'
import { loadContent } from '../shared/content.ts'
import { documents } from './fixtures.ts'

const components: Component[] = [
  { id: 'app', name: 'App', kind: 'service', dependsOn: ['core'] },
  { id: 'capture', name: 'Capture', kind: 'module', parentId: 'app' },
  { id: 'buffer', name: 'Audio buffer', kind: 'local-storage', parentId: 'capture' },
  { id: 'editor', name: 'Editor', kind: 'module', parentId: 'app' },
  { id: 'core', name: 'Core', kind: 'service', dependsOn: ['postgres', 'provider'] },
  { id: 'postgres', name: 'Postgres', kind: 'database' },
  { id: 'provider', name: 'Provider', kind: 'external-service' },
]
const feature: Feature = { id: 'f', productId: 'p', title: 'Capture', stage: 'building', ownerId: 'ryan', owner: 'Ryan', touches: ['buffer'],
  updatedAt: '2026-09-05T00:00:00Z', spec: {
...
test('invalid containment and dependency references are rejected before rendering', () => {
  ...
  check(d => { d[cart].parentId = 'c-cart-cache';
    d[cache].parentId = 'c-cart-svc' }, /containment cycle/)
  check(d => { d[cache].parentId = 'c-pricing-api' }, /same product/)
  check(d => { d[cache].kind = 'database';
    d[cart].parentId = 'c-cart-cache' }, /only a service or module/)
  check(d => { d[cache].kind = 'external-service';
    d[cache].parentId = 'c-cart-svc' }, /must be top-level/)
...
test('runtime system graph separates supporting modules without hiding isolated services', () => {
  const runtime: Component = { id: 'runtime', name: 'Runtime', kind: 'service' }
  const isolated: Component = { id: 'docs', name: 'Docs', kind: 'service', dependsOn: [] }
  const supporting: Component = { id: 'platform', name: 'Platform', kind: 'module', dependsOn: ['runtime', 'docs'] }
...
test('architecture system graph adds observed infrastructure without promoting it to a component', () => {
  ...
  const core: Component = {
    id: 'core', name: 'Core', kind: 'service',
    unresolvedDependencies: [
      { name: 'PostgreSQL', kind: 'database', transport: 'TCP', evidence: [{ path: 'db.go', lines: '1', claim: 'Connects to Postgres', revision: 'abc' }] },
      { name: 'Events', kind: 'message broker', transport: 'Pub/Sub', evidence: [{ path: 'events.go', lines: '1', claim: 'Publishes events',
        revision: 'abc' }] },
    ],
  }
...
test('architecture map points inbound event brokers toward consumers and outbound events toward brokers', () => {
  const service: Component = {
    id: 'facade', name: 'Facade', kind: 'service',
    unresolvedDependencies: [
    ...
    messaging: { messages: [
      ...
      { id: 'reply', name: 'Reply', broker: 'Kafka', channel: 'replies', direction: 'outbound', fields: [], delivery: { ordering: 'Unknown',
        retries: 'Unknown', deadLetter: 'Unknown' } },
    ] },
  }
...
test('architecture map shows a single bidirectional broker link when both directions are catalogued', () => {
  const app: Component = {
    id: 'app', name: 'App', kind: 'service', dependsOn: ['broker'],
    messaging: { messages: [
      { id: 'consume', name: 'Consume', broker: 'Kafka', channel: 'input', direction: 'inbound', fields: [], delivery: { ordering: 'Unknown',
        retries: 'Unknown', deadLetter: 'Unknown' } },
      { id: 'publish', name: 'Publish', broker: 'Kafka', channel: 'output', direction: 'outbound', fields: [], delivery: { ordering: 'Unknown',
        retries: 'Unknown', deadLetter: 'Unknown' } },
    ] },
  }
  const broker: Component = { id: 'broker', name: 'Kafka', kind: 'queue' }
...
test('a gateway map includes inbound and outbound brokers once each', () => {
  const gateway: Component = { id: 'gateway', name: 'Gateway', kind: 'service', messaging: { messages: [
    { id: 'out', name: 'Out', broker: 'Kafka', channel: 'out', direction: 'outbound', fields: [],
      delivery: { ordering: 'Unknown', retries: 'Unknown', deadLetter: 'Unknown' } },
    { id: 'in', name: 'In', broker: 'Partner Kafka', channel: 'in', direction: 'inbound', fields: [],
      delivery: { ordering: 'Unknown', retries: 'Unknown', deadLetter: 'Unknown' } },
  ] } }
  ...
  assert.deepEqual(graph.edges.filter(edge => edge.messages).map(edge => [name(edge.from), name(edge.to), edge.messages]), [
    ['Gateway', 'Kafka', { inbound: 0, outbound: 1 }],
    ['Partner Kafka', 'Gateway', { inbound: 1, outbound: 0 }],
  ])
})
test('product map retains cross-product dependencies without pulling in their whole system', () => {
  const input = structuredClone(components)
  const external: Component = { id: 'other', name: 'Other service', kind: 'service', dependsOn: ['unused'] }
  input.find(c => c.id === 'buffer')!.dependsOn = ['other']
  const graph = systemGraph(input, [...input, external, { id: 'unused', name: 'Unrelated', }])
...
test('product maps include external users while leaving unrelated dependencies out', () => {
  const external: Component = { id: 'consumer', name: 'Consumer', kind: 'service', dependsOn: ['buffer', 'unused'] }
  const graph = systemGraph(components, [...components, external, { id: 'unused', name: 'Unrelated', }])
