...
| `0.17-pre-doc-shape-advance/` | Synthetic, minimal (docs only — not a full install): the pre-C-shape canonical docs the `product-brief-shape` and `design-system-shape` reconcile families advance from. `docs/product-brief.md` carries all eight retired sections with real prose in each; `docs/design-system.md` is a full greenfield spec with the envelope under `# Part 1 — Constraints` and no `Commitments` or `## Pattern Index` heading, and no `Graphical UI` type-section title for the registry's `design track` fields to resolve to. `docs/surfaces.md` and its `.groundwork/surfaces.json` twin carry two active surfaces and an empty capability ledger, so the brief advance's registry branch has a real target. |
