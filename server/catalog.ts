...
function summary(plan: Plan, entry: Entity) {
  ...
  if (typeof entry.raw.source === 'string' && !pointers.some(pointer => pointer.path === entry.raw.source)) pointers.unshift({ repository: component.repo ?? null, path: entry.raw.source, revision: sourceRevision ?? '', lines: '', claim: 'Starting location; inspect implementation before claiming behavior.', url: sourceRevision ? sourceEvidenceUrl(component.repo, { path: entry.raw.source, revision: sourceRevision, lines: '1', claim: '' }) ?? null : null })
  const flow = entry.kind === 'flow' ? entry : null
  const traced = entry.kind === 'endpoint' ? entry.related.some(relation => relation.reason === 'Recorded execution path') : null
...
function rank(entries: Entity[], query: string) {
  const terms = [...new Set(tokens(query).filter(term => !stop.has(term)))]
  return entries.map(entry => {
    ...
    const matches = terms.filter(term => title.includes(term) || body.includes(term))
    const score = (matches.reduce((sum, term) => sum + (title.includes(term) ? 5 : 3), 0) + (matches.length === terms.length && terms.length > 1 ? 15 : 0)) * (entry.kind === 'schema' ? 0.6 : 1)
    return { entry, score, reasons: terms.length ? matches.map(term => `Text match: ${term}`) : ['Catalog inventory order'] }
  }).filter(result => !terms.length || result.score > 0).sort((a, b) => b.score - a.score || a.entry.id.localeCompare(b.entry.id))
}
...
export function queryCatalog(plan: Plan, operation: 'search_catalog' | 'get_catalog_entity' | 'get_discovery_context', input: unknown) {
  const args = (operation === 'search_catalog' ? searchCatalogSchema : operation === 'get_catalog_entity' ? getCatalogEntitySchema : discoveryContextSchema).parse(input)
  const { cursor, ...binding } = args
  ...
  if ('id' in args) {
    parseCatalogId(args.id)
    const entry = entries.find(entry => entry.id === args.id)
    if (!entry) throw new Error('Catalog entity not found in the selected snapshot')
    items = detailParts({ entity: entry.raw, source: { repository: entry.component.repo ?? null, revision: catalogSourceRevision(entry) }, relations: entry.related, knowledge: catalogKnowledgeState(entry.component), gaps: entry.component.gaps ?? [] })
    totalCandidates = 1
  } else {
    const results = rank(filtered(entries, args), 'query' in args ? args.query : args.question)
    totalCandidates = results.length
    if ('seeds' in args) {
      const eligible = filtered(entries, args)
      const seeds = args.seeds.map(id => { parseCatalogId(id); const entry = eligible.find(entry => entry.id === id); if (!entry) throw new Error(`Seed not found within the selected scope: ${id}`); return { entry, score: 1000, reasons: ['Explicit seed'] } })
      ...
      items = [...ordered, ...related].filter(result => !seen.has(result.entry.id) && !!seen.add(result.entry.id)).map(result => ({ ...summary(plan, result.entry), matchReasons: result.reasons.slice(0, 5) }))
    } else items = results.map(result => ({ ...summary(plan, result.entry), matchReasons: result.reasons.slice(0, 5) }))
  }
  if (offset > items.length) throw new Error('Catalog cursor is out of range')
  const page: unknown[] = []
  const envelope = (nextOffset: number) => ({
    ...
    limits: { maxBytes: args.maxBytes, limit: args.limit, relationDepth: operation === 'get_discovery_context' ? 1 : 0 },
    total: items.length, totalCandidates, offset, items: page,
    omitted: items.length - nextOffset,
    nextCursor: nextOffset < items.length ? Buffer.from(JSON.stringify({ snapshot, query, offset: nextOffset })).toString('base64url') : null,
    detailFormat: operation === 'get_catalog_entity' ? 'JSON-pointer sections; stringOffset/stringLength identify sliced strings. Follow every page for full detail.' : 'Summaries; use get_catalog_entity for complete evidence and relations.',
  ...
  for (const item of items.slice(offset, offset + args.limit)) {
    page.push(item)
    if (Buffer.byteLength(JSON.stringify(envelope(offset + page.length))) > args.maxBytes) { page.pop(); break }
  }
  if (!page.length && offset < items.length) throw new Error('Catalog item exceeds maxBytes; increase the byte limit')
