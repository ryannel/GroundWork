...
import { z } from 'zod'
import { featureSchema, featureStageSchema } from '../shared/content-schema.ts'
import { idPattern } from '../shared/schema-primitives.ts'
...
export function recordProgress(plan: PlanState, args: RecordProgress, now = new Date().toISOString()): Changes {
  ...
  if (args.unitId && args.status) {
    const unit = [...delivery.tasks, ...delivery.deliverables].find(item => item.id === args.unitId)
...
export const linkBranchFields = { featureId: stableId, taskId: stableId.optional(), branch: z.string().min(1) }
...
export function linkBranch(plan: PlanState, args: LinkBranch, localBranches: string[], now = new Date().toISOString()): Changes {
  ...
  const delivery = currentDelivery(plan, args.featureId)
  const link = { branch: args.branch, ...(args.taskId ? { taskId: args.taskId } : {}) }
  const linked = delivery.branches.some(item => item.branch === link.branch && item.taskId === link.taskId)
