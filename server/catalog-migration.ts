...
export async function migrateCatalog(root: string, input: unknown) {
  ...
  if (args.expectedRevision !== plan.revision || args.expectedContext !== plan.context.token) throw new Conflict('Apply migration with the current dry-run revision and context')
