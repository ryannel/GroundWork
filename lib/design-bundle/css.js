...
// values that differ. Per-theme values are palette and surface treatments; type,
// elevation, shape, blur and motion are theme-agnostic in this token model, so
// they are emitted once.
...
function staticVars(v) {
  ...
  for (const [level, shadow] of Object.entries(v.shadow)) {
    lines.push(`  --shadow-${level}: ${shadow};`);
  }
