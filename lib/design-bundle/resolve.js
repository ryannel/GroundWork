...
const NEUTRAL_SHADOW = {
  low: '0 1px 2px oklch(0% 0 0 / 0.06)',
  mid: '0 1px 2px oklch(0% 0 0 / 0.06), 0 4px 8px oklch(0% 0 0 / 0.04)',
  high: '0 1px 2px oklch(0% 0 0 / 0.06), 0 4px 8px oklch(0% 0 0 / 0.04), 0 12px 24px oklch(0% 0 0 / 0.03)',
};
...
function resolveVisual(cwd) {
  ...
  if (v.elevation && typeof v.elevation === 'object') {
    ...
    for (const [level, layers] of Object.entries(v.elevation)) {
      const composed = composeShadow(layers);
      if (composed) shadow[level] = composed;
    }
