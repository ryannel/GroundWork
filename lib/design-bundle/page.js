...
//   - Every card shows its id in monospace, so the owner can name a specific
//     card when they react to it. They react in the CONVERSATION, not in a file:
//     the agent is present, and a chat interface is the one thing every agent
//     host has. Feedback that needs to outlive the session is a decision, and
//     the decisions queue already exists for that.
...
function renderIndex(cards, tokensCss, { appName, warning }) {
  const groups = [];
...
<main>${body}</main>
<footer>
  Say what you think — name a card by the id shown beside it. Anything you ask for is a change to <code>.groundwork/config/brand-tokens.json</code>, and this page is rebuilt from that file, so you will see the result rather than be told about it.
</footer>
</html>
`;
}

module.exports = { renderIndex, PAGE_CSS };
