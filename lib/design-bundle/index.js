...
//
// There is deliberately no critique file. The owner reacts in the conversation:
// the agent is already there, a chat interface is the one capability every agent
// host has, and a reaction that needs to outlive the session is a decision — for
// which `groundwork decisions` already exists. How the sheet reaches the owner is
// the host's business: render it inline where the host can, hand over the path
// where it cannot.
//
...
function build({ cwd = process.cwd(), slug = null } = {}) {
  ...
  fs.writeFileSync(
    ...
    renderIndex(cards, tokensCss, { appName: visual.appName, warning })
  );
