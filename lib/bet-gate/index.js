...
function unitContentChecks(label, mdPath) {
  ...
  // A named Test file: a "Test file:" line (or any backtick-quoted path) naming a
  // real test file — a test/spec marker plus a source extension. Root-agnostic so
  // non-JS/TS projects name their own test roots (Swift Packages/Tests/…Tests.swift,
  // Go *_test.go, …), while still catching a missing or placeholder line.
  const looksLikeTest = (p) => /(test|spec)/i.test(p) && /\.[A-Za-z0-9]+$/.test(p);
  const labelled = text.match(/Test file:\*{0,2}\s*`?([^\s`]+)`?/i);
  const quoted = text.match(/`([^`]*(?:test|spec)[^`]*\.[A-Za-z0-9]+)`/i);
  const namesTest = (labelled && looksLikeTest(labelled[1])) || !!quoted;
  checks.push(check(`${label}: Test file named`, !!namesTest,
    `${mdPath} names no test file (add a "Test file:" line naming a real test file)`));
  return checks;
}
...
function decompositionChecks(targetDir, slug, scope = {}) {
  ...
  if (meta && Array.isArray(meta.pages)) {
    // A page resolves as either a file (<rel>) or a milestone directory (<rel>/index.md),
    // since the milestone template renders NN-<slug>/index.md and meta may list the dir.
    const dangling = meta.pages.filter((rel) => {
      if (typeof rel !== 'string') return false;
      const abs = path.join(decomp, rel);
      return !isFile(abs) && !isFile(path.join(abs, 'index.md'));
    });
