...
// often an ABI mismatch (the bundled wasm targets a newer/older tree-sitter than
// the installed web-tree-sitter), or a project-supplied wasm that does not exist.
// We cache the failure and let the caller degrade: that language is skipped and
// reported, never a crash. Queries are optional — a 'symbols' language may have
// no import query, and some have no symbol query.
async function getLanguage(lang) {
  ...
  try {
    ...
    const language = await Parser.Language.load(langs.grammarPathFor(lang));
    // setLanguage is where an ABI mismatch actually throws — probe it now so the
    // failure is caught here rather than on the first parse.
    const probe = new Parser();
    probe.setLanguage(language);
    probe.delete();
    entry = {
      language,
      importQuery: def.importQuery ? language.query(def.importQuery) : null,
      symbolQuery: def.symbolQuery ? language.query(def.symbolQuery) : null,
    };
  } catch (err) {
...
async function generate({ cwd, cacheDir }) {
  ...
  return {
    ...
    cache,
    stats: { parsed, cached },
    diagnostics: { projectLanguages: project.loaded, projectErrors: project.errors, unmapped },
  };
}
