...
const conventionsMod = require('./conventions');
...
// Submodules enumerated from .gitmodules. `git ls-files` does not descend into
// gitlinks, so submodule contents are absent from the map by construction —
// enumerating them keeps that absence visible instead of silent. Consumers (the
// scan, Serena) read submodule working trees in place.
function listSubmodules(cwd) {
  const file = path.join(cwd, '.gitmodules');
  ...
  return out
    ...
    .map((s) => {
      ...
      try {
        initialized = fs.readdirSync(path.join(cwd, s.path)).length > 0;
      } catch {}
      return { path: s.path, url: s.url, initialized };
    });
}
...
// The deterministic conventions digest (runtimes, commands, hubs, layout) over
// the same tracked-file view generate() maps. Pure of wall-clock and machine
// state by construction — see lib/repo-map/conventions.js.
function buildConventions({ cwd, map }) {
  return conventionsMod.conventions({ cwd, files: listRepoFiles(cwd), map });
}
...
module.exports = {
  ...
  langForFile, // consumers that read a written map back (generate-views) need it too
  buildConventions,
  renderConventionsMarkdown: conventionsMod.renderMarkdown,
  summarizeConventions: conventionsMod.summarize,
  SCHEMA_VERSION,
};
