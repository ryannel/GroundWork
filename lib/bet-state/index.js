...
function listFindings(targetDir, slug, { status, milestone, slice } = {}) {
...
function listDecisions(targetDir, slug, { status } = {}) {
  const doc = readDoc(decisionsPath(targetDir, slug), slug, 'decisions');
  return doc.decisions.filter((d) => (status ? d.status === status : true));
}
