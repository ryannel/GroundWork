...
A quoted line counts only inside a **cluster**: 2 or more trailer-shaped lines next to each other (D64 ruling 7). That is the shape a squash quotes, because it quotes a whole trailer block.

Trailer-shaped means the key is one this repo writes as a trailer, and there are three of them: `Bet`, `Slice`, `Tests`. The working agreement fixes the first and last, and section 3 fixes the middle one. Any other word before a colon is prose.

So the two shapes a reader has to tell apart come out this way. A `Notes:` label above a sentence starting `Slice:` is one trailer-shaped line, not two, because `Notes` is not a trailer key — green. `Bet:` and `Slice:` on adjacent lines are two, whatever the prose above them says — red. The second is the honest answer even when the commit meant well: the board cannot read either line, so the slices they name are unlanded as far as anything here can tell.

**A warning for whoever writes a commit message.** Reproducing a whole trailer block verbatim at the start of a line reads as a squash, and history does not change, so that red never clears. Quote a trailer block with indentation or inside a fence.
...
A message longer than 65536 bytes is read only in part. The row judges the part it read and counts the message as cut, so a squash quoted inside the first 65536 bytes is still `1 squashed, 1 cut`. The count is the reader's warning that a verdict rests on part of a message.
...
The `waiver-count` row's counts are the waiver files read, the grants counted, the merges it did not read, the renames it did not read, the rows at a limit, and the grants misstated. The count of files that are not waivers is not among them: each is named on this row's line, and the run's own loud block names every one again with the reason. Each row at a limit says on its own line whether a finding answers it, and the rows no finding answers lead the line (D64 ruling 8).
