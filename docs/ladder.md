...
**Status:** Ratified 2026-08-22, with the spec (#31). Seventeen bets, with every commitment in the spec assigned to one. Hardened by six independent reviews before ratification.
**Audience:** The owner ratifying the sequence, and the driver running it.
**Scope:** Order and coverage. How we deliver is [the execution plan](execution-plan.md); what we build is [the spec](spec/index.md).
...
Each item has an ID: `B0`–`B44` for build, `K0`–`K22` for keep, `D0`–`D17` for delete, `O0`–`O35` for open. Each ID marks a position in [changes.md](spec/changes.md)'s lists. That means the coverage check can run as a script — nobody has to check it by eye.

**Terms.** The bets use the spec's own words — kernel, dial, lane, seal, socket, blessed module, front door, and more. They're defined under "The words this set uses" in [index.md](spec/index.md). Read that first if any are new to you. Magpie and staycurrent are existing products built with the old framework. Wordloop is a fourth repo that predates it. All three show up as test fixtures.
...
**Nothing is unassigned.** 122 of 122 items have exactly one bet, checked by script against [changes.md](spec/changes.md).
