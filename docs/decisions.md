...
1. The landing rule. The oldest claim stands — D57.4 unchanged. The contract states the writing convention: no Slice trailer rides a commit before the landing. The board gains a fourth flag, "landing not proven". It fires only when a required proof is red, and HEAD is that slice's own landing commit. It is counted apart, named as a note, and never red. The walk found something else too. No trailer-ordering rule can fix F124. At a red-proof commit, the oldest and newest claim are the same commit. F120 and F124 are one fault at two commits and two different fixes. The cost is named: commits between a red commit and its landing still read falsely behind.
2. The stub catch. The catch: a bare-identifier alias, traced one hop, inside the honesty scan's self-comparison check. It reads both directions, in straight-line code only. Any write or escape in between drops it. No sixth shape name, so D54 holds. Named limits: two hops, arithmetic identities, selector aliases, helper-returned copies, two literals.
3. Ladder versus contract. The contract is truth and the milestone is the unit, because sealed R10 says so. This ruling corrects the ladder's bet 3 clause. The new clause is stricter: no board goes red on a repo that commits its tests first. The correction follows a failed grade and says so where a reader meets it.
4. The wiring sentence. The constant is reworded to state the rule that condemned the hit. It now reads: on profile library an export is dead only when nothing at all names it. Position unchanged — the line has no room left for a clause it can never drop.
...
Why: the walk did what a walk is for. It split a fault the ledger had fused. F120 and F124 wanted different fixes. The deferred ruling would have fixed one and shipped the other unfixed.
