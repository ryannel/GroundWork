...
| Sets the model on a dispatched subagent (Claude Code: the `Task` tool's `model`) | Running gated execution a tier cheaper than the planning/review tier (operating contract, Model Tiers) | Degrades: a host that pins one session model runs **everything at the `frontier` tier** — correct but pricier, never weaker. Review is never silently downgraded. |
| Lets a running agent escalate to a stronger model mid-task (Claude Code: the **advisor**, `advisorModel`/`/advisor`; subagents inherit it) | An `execution`-tier worker consulting a `frontier` model when it is battling a slice, instead of grinding toward a forced green (operating contract, Model Tiers — *Runtime escalation*) | Degrades: without it, a stuck worker falls back to its `BLOCKING CONCERN` hand-back and the frontier review gate still catches a weak green. Optional, not required. |
...
| Renders a local file inline in the session (Claude Code: the plan/artifact review surface) | Putting the generated design sheet (`groundwork design build`) in front of the owner so they seal a look they saw, not a token string; the same surface Protocol 12 uses for draft review | Degrades: the sheet is a self-contained HTML file at a canonical path, so the agent hands over the path and the owner opens it themselves. Their reaction comes back through the conversation either way, which every host has. Nothing about the seal depends on the rendering surface. |
...
GroundWork assigns each dispatched subagent a **model tier** by the reasoning its job needs (operating contract, *Model Tiers* — the canonical policy). The driver's planning and **every** review lens run at a **`frontier`** tier ("Opus-class" — the host's strongest reasoning model); gated execution (the slice and reconcile workers) runs at a cheaper **`execution`** tier ("Sonnet-class"). The principle is that workers can run cheap only because the independent review is the gate — so the gate is never the weak link. A slice flagged *challenging or vague* at authoring lifts its worker to `frontier` for that slice.

Tiers name a **class of model, not a model id** — described by capability with a few exemplars (Claude Opus/Sonnet · Gemini Pro/Flash · GPT-5 / GPT-5-mini classes), so the agent picks whatever its host offers that fits, and the policy never goes stale on a release. Claude Code is the proven reference host; the cross-provider exemplars are illustration, not a claim of first-class support (see the matrix above). On a host that cannot set a per-subagent model, the policy degrades to all-`frontier` per the capability table.

The cheaper `execution` tier stays safe in practice because a worker is never left alone with a problem above its weight: one that is battling a slice **escalates to a `frontier` model mid-task** rather than grinding toward a forced green. On Claude Code this is the built-in **advisor** — set `advisorModel` to a frontier-class model (e.g. `/advisor opus`) and dispatched subagents inherit it, so a Sonnet worker consults an Opus advisor at decision points (the advisor is experimental and Anthropic-API-only; advisor tokens are billed separately). Where a host offers no such mechanism, a stuck worker still has its `BLOCKING CONCERN` hand-back and the frontier review gate behind it.
...
Supported agent keys: `claude-code`, `cursor`, `codex`, `opencode`, `cline` — the machine-readable registry is `src/config/hosts.json`, and a contract test keeps its keys and statuses in sync with the Support matrix above. Claude Code gets `.claude → .agents` and `CLAUDE.md → AGENTS.md`; the `AGENTS.md`-native cluster reads the canonical files directly. The selection is recorded in `.groundwork/config/state.json` so `update` self-heals the same links. Add an agent later with `npx groundwork-method init --agent <name>` — non-destructive.
