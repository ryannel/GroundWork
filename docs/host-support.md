...
| Node — `npx groundwork-method repo-map` (bundled tree-sitter, no network at run time) | The deterministic code map (`repo-map.json`): import edges + PageRank centrality for Go/Python/TS/JS | Degrades: for an unsupported language, or where the generator cannot run, the scan infers the map's missing parts via LLM in the same shape. Refresh is detect-and-lazy; a git hook is opt-in only. |
