...
**The design system.** A product with a UI gets its design system before its screens ([standards.md](standards.md)), and it is sealed rendered ([loop.md](loop.md)): as the tokens are written, the agent generates a sheet from them — palette in both themes, type at real size, elevation and motion demonstrated — and the owner explores and seals a look they saw, never token strings on faith. Because the sheet is generated from the token file rather than drawn by hand, what the owner approves and what the app compiles are the same values. A hosted canvas, where one is available, is somewhere to browse the result afterwards; the seal never depends on it.
