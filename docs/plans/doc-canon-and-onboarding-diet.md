...
**Status:** WS-A–WS-F EXECUTED + WS-G CORE ARMS GREEN 2026-08-13 — WS-C (`d98dcad`, `f4562c2`), WS-F (`1c803b5`), WS-D (`b49d819`), WS-E (`6c99e1c`), all pushed; opus subagents implemented D/E/F under driver review. WS-G verification: **G4′ new-track brownfield arm green** (patch → hand-off+deferral → scan-lite with the new review gate rejecting an overclaiming first draft → ops-adopt authoring registry+harness), **G2 upgrade reconcile green** (both families, verbatim preservation, honest empty ledger, gate caught a real conformance gap), **G3 greenfield green** (342-word brief, Commitments + Pattern Index anchors live). Graded findings: `.sandboxes/first-contact-review/findings.md` (15 findings, 3 runs + 3 arms). **2026-08-14 close-out:** the methodology_twin full-arc run (sim-20260814-104901, 11 commits, 5/5 docs solid) proved the full new track to graduation AND post-graduation deferred re-entry live (convergence ran after graduation: inventory regenerated, P13 sanction, convert/keep dispositions, ADR-0002, register cleared, durable marker) — the re-entry arm is discharged with one thin residual (the bet-triggered routing sentence; carried as v2 fixture material). Harness follow-ups landed (`f573397`: stall stamp un-sticks, caffeinate guard). **Still owed (deliberately deferred to v2's evidence ladder):** G1 orientation eval, G5 live-repo proof (magpie / wordloop-attended). Earlier: WS-A + WS-B EXECUTED 2026-08-13 (commits `4e2302e` B1, `0f1f5f9` B2+B3, `25f020c` A1+A2, `315dbf8` A3; 320 cli tests + lint + sync-anchors green). Execution notes: (1) B2's default-track flip is **not** in — it references WS-E phases that do not exist yet; what landed is the full deferral semantics (settled-phase Mode Detection, graduation predicate, deferred re-entry incl. the deferrable-set guard: only the product-brief and design-system extracts qualify pre-E, since scan feeds the extracts and architecture-extract feeds the ops harness) with the track flip staying in WS-E. (2) The R5 interim guard landed: infra-adopt preserves the scan cache while any extract is deferred; the last deferred extract owns teardown — superseded by E4's regenerate-at-invocation when WS-E lands. (3) A1's acceptance shifted: orchestrator routing is skill prose, not CLI code, so the planned `tests/cli` routing table-test is not mechanically expressible — coverage is the new progress-header lint rule plus the G4 sim probes. (4) **G4 first bounded arm RAN 2026-08-13** (sim-20260813-154410, sonnet, bounded at Codebase Scan, graded findings in `.sandboxes/first-contact-review/findings.md`): A1 provisional patch **passed outright** — opening bug ask shipped in session one as a stamped `Lane: patch` commit, blind reviewer caught a real adjacent bug; scan honesty and duration framing passed. Two gaps found and fixed same-day (`983e67f`): the hand-off now anchors to setup-track *entry* (it previously skipped the after-a-provisional-patch entry, which also left the deferral write-path unexercised), and the orchestrator supplies the phase-N-of-M position line at routing time. (5) **Deferral re-probe RAN 2026-08-13** (sim-20260813-161808, sonnet, post-fix sandbox): the re-anchored hand-off fired at setup-track entry — five-phase road in owner language, deferrable phases named, an honest answer to "does this block future fixes?" — and the persona's deferral of both extracts landed in `phase_states` with notes; the session's own close names Architecture Extract as the next honest route, skipping the deferred phases. One drift found and fixed (`state` key of the register entry pinned in SKILL.md — the sim wrote `status`, the test fixture `state`; the prose had never pinned the key). Still owed from G4: the full post-graduation re-entry arm (deferred extract invoked when a later bet needs its artifact). (6) **D-O5 RULED + WS-C EXECUTED 2026-08-13** (commits `d98dcad` C1+C2, `f4562c2` C3+C4+C5; lint + sync-anchors green). Scoping calls made in execution, both red-team-consistent: the greenfield design system keeps its full prescriptive translation at setup — no code exists yet, so the spec is the canon (the R3 staging logic applied to design) — while gaining the same named anchors as the lean brownfield shape (`## Commitments` pinned in both draft layouts, seeded `## Pattern Index`); and the brief's discovery depth now travels in the hand-off file, formally declared load-bearing (capability inventory, experience journeys, user mental models) so greenfield downstream phases aren't starved by the half-page committed shape. New identifier pins: `Commitments` heading (three gates cite it; checklist guards it), type-section titles (already pinned), Pattern Index heading (design step + auditor anchor). Remaining: WS-D…WS-G per sequencing. Derived from two subagent review suites run the same day: a six-lens onboarding review of the brownfield flow (14 agents, 40 findings, top 8 adversarially verified: 4 confirmed / 4 partial / 0 refuted) and a six-lens red-team of the resulting doc-cut proposals (14 agents, 55 findings, top 8 verified: 3 confirmed / 5 partial / 0 refuted). Full reports: [onboarding review](https://claude.ai/code/artifact/ed972628-924b-4c4e-b672-11c1d6c9b49a), [doc-diet red-team](https://claude.ai/code/artifact/dab24136-9fa0-49f6-974b-0cbdbd3a9e30).
