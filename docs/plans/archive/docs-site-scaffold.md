...
**Status:** EXECUTED 2026-06-19 (WS-A, WS-B, WS-C, WS-D) + polish pass. The `docs-site` generator now serves the live `docs/` tree (bets included, H1-derived titles, bets-first nav) as a native `./dev` runner — verified against a real `next build` (7 static pages, all bet/lifecycle routes; titles confirmed in prerendered HTML), a green compilation test (no longer `xfail`), and a **full runner-lifecycle boot test** (`test_scaffolds.py::test_10` — `./dev start` boots `pnpm dev` and serves `/docs` HTTP 200). Polish pass also fixed the pnpm 10+ build-script gate (a shipped `pnpm-workspace.yaml` so `pnpm install`/`pnpm dev` exit 0), an honest post-gen install hook, and trimmed Tailwind globs. WS-E (Mintlify `--engine` seam) remains deferred per its gating (O4). Originally PROPOSED 2026-06-17; **updated 2026-06-18** onto the now-complete scaffold foundation from `dev-cli-native-runners.md` — the native-runner registry, the `registerRunner()` generator helper, the capability-port/footprint framework, and the `infrastructure.md` footprint matrix all shipped there. Triggered by a request to "support a doc site as part of the scaffold so the bets are easily viewable," using Fumadocs (with Mintlify floated as a possible second option). **The generator already exists** (`src/generators/docs-site/`, registered in `generators.json`, mapped in `groundwork-scaffold` phase 01, covered by a generation test). It is *not* a greenfield build — it is an unfinished one, and it is now the **only surface-shaped generator still injecting into `docker-compose.yml`** while `electron-app`/`flutter-app`/`cli-app` already self-register as native runners. This plan closes the gap between "a Fumadocs project is generated" and "a docs site that actually serves `docs/` well, surfaces bets, runs cleanly, and is offered as a real optional scaffold step." Picks up `G3 docs site` left open in `contract-grade-delivery.md`; the native-runner foundation it depends on is no longer pending.
...
- **C4 — Capability discovery (DONE 2026-06-18).** Independent of whether docs-site is generated, the always-on orchestrator must be able to *answer* "can we scaffold a docs site?" without entering the scaffold flow. Added a capability-discovery intent handler to `src/skills/groundwork-orchestrator/SKILL.md` that answers from the shipped catalog `.groundwork/config/generators.json` (name + description per generator), with flag-level detail read read-only from the phase-01 mapping table — no second catalog to drift. *Acceptance:* a user asking about a generator gets a correct yes/no + description from the deployed catalog; verified the docs-site entry reads clearly there. CHANGELOG `[no-migration]` line added (skills are Tier-1, clean-replaced on update).
