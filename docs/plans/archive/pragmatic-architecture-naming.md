...
**Status:** EXECUTED 2026-06-19 (pending human-run live sims — WS-I9). Discovery (Part A) complete; all §5 decisions settled (below). Part B executed end-to-end: WS-I1–I8 landed and gated green — Go scaffold (interfaces in `internal/core/service`, technology-named adapter packages, **real depguard proven to fail on a core→edge import**), Python scaffold (`core/ports.py` + `src/adapters/`, **real import-linter**), the shared capability layer + cross-phase contract de-jargoned (machine keys kept stable), engineer skills + mirrors + sync-anchors realigned, brownfield extractor updated, and the owed `gw-code-structure-rename` agent migration authored. Gates green: Go gen (19) + compile (9) + add-capability (2); Python gen (131); Next/Flutter/Electron gen (51); contracts (6); cli (40); migration gate (10); sync-anchor + mirror + init. **Not done here:** WS-I9 live greenfield/brownfield simulations (billed, need eval credits + Docker — out of this environment); `package.json` version bump (held for the release act). Nothing committed; work sits uncommitted on `feat/dev-cli-native-runners` for review.
...
### Settled (resolved 2026-06-19, were D-OPEN)
| ID | Decision | Notes |
|---|---|---|
| D-OPEN-1 | **Resolved in Phase 1.** Principle named `system-design/code-structure.md` / "How We Structure Code" (a pure core, swappable edges, one obvious place). `hexagonal-architecture.md` deleted; persona ref → `core-and-boundaries.md`. | Done. |
| D-OPEN-2 | **Go:** drop `internal/core/gateway/` (interfaces into the consuming `internal/core/service` package) + split `internal/provider/` into technology-named packages (`postgres`, `kafka`, `pubsub`, `httpclient`, `websocket`, `llm`); `LLMGateway`→`TextGenerator`, adapter `llm.Client`. **Python (user decision):** cosmic-python idiom — `core/domain/protocols.py`→`core/ports.py`, `src/provider/`→`src/adapters/`, tech-prefixed impl classes. The `src/<pkg>/` PEP src-layout move followed on 2026-06-20 (importable package = service name in snake_case; the capability injector derives the package from `pyproject.toml`, so no dynamic-package plumbing was forced on `add-capability`; proven by a real `uv sync` + `import <pkg>.main` compilation case). **Next.js:** already idiomatic — prose-only. Full contract in `target-scheme.md`. | D-SET-7 honoured: stacks diverge by idiom. |
| D-OPEN-3 | **Document-only** for users' hand-evolved service code (their code is theirs); migrate GroundWork-owned artifacts + drive correctness in new generation. Plus the **owed** `gw-code-structure-rename` agent migration for the Phase-1 filename move (`hexagonal-architecture.md`→`code-structure.md`). | Adopted. |
| D-SET-8 | **Add real CI enforcement** (orchestrator decision, 2026-06-19): `depguard` (Go) + `import-linter` (Python) ship as working gates that fail on an inward-flow violation. Discovery found both were prose-only fiction; shipping them makes the corpus's "enforceable in CI = guarantee" claim true. | Acceptance: a deliberate core→edge import fails the linter. |
| D-SET-9 | **Keep machine keys stable:** the `CapabilityStack` field keys (`port`/`adapter`/`portSymbol`/`adapterSymbol`) and the `.groundwork/capability-ports.json` filename + `ports` key are unchanged. No code reads the latter; the former is internal plumbing. De-jargon only the prose + update the values. | Collapses install migrations to the one doc rename. |
