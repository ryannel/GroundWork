...
>
> **Remaining:** WS-D (scaffold reconciliation), WS-E (migration/changelog + Docker boot test `./dev test scaffolds`), WS-F (composable capability ports & providers). Not yet committed; CHANGELOG `[no-migration]` line owed (changes are additive for existing installs). Full generation suite (216 tests, ~1h on this machine) not re-run after WS-C — targeted suites green; worth a full pass before commit.
