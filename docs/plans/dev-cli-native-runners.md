...
>
> **Remaining:** WS-F rounding-out — F6 (engineer-skill capability-ports reference), F7 (architecture phase declares capabilities + `capabilities.json` machine twin), F8 (scaffold reconciliation, folds into WS-D), F9 (footprint-matrix tests for compose/runner kinds), F5/O9 (Go + Next.js provider families). WS-D (scaffold reconciliation), WS-E (Docker boot test `./dev test scaffolds`, retro-registration migration WS-E1).
