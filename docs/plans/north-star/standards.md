...
- **Testing**: honeycomb shape. System tests first, unit tests where logic warrants. Independent oracles: the author never grades its own work. Tests are written first and observed failing before the implementation exists. Real infrastructure over mocks at the seams.
