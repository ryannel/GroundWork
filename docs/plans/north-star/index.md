...
- `dated`: works around a current model weakness. Carries a review-by date, six months maximum. At the date: re-justify with evidence or delete. The evidence differs by kind, because a preventive rule's success is invisible in fire counts — a forcer that always works never fires, the same trap as a linter that never fires. Detective checks are judged on the journal's counts: fired, caught, waived. Preventive rules are judged on the recurrence of the defect class they prevent, read from the findings ledger's class tags, and by the switch-off experiment below — which is scarce, so review dates are tiered: items the journal can answer renew on their counts, and the off-switch queue is reserved for the few it cannot.
...
8. The upkeep share. The journal publishes framework-maintenance spend as a share of total spend; a rising trend is the 384-commit treadmill returning — this time in code instead of prose.
