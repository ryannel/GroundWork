...
3. On the complex lane, the blind test author writes the accepting suite ([proof.md](proof.md)). Its inputs: the sealed design, the proof plan, and the public interface of the handed-off code — names, signatures, endpoints, extracted mechanically. Never the code bodies, never the implementer's tests. Failures triage through the driver: an implementation bug goes back to the implementer with the failing output; a test that misread the design is fixed against the design.
...
8. A fresh-context subagent writes the capsule, handed the facts it needs — the diff, the proof plan, the findings — so it renders them into plain prose and never goes researching. The slice enters the Queue unless a prior seal covers it.
