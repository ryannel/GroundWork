...
**Status:** PROPOSED 2026-06-17. Triggered by a request to "support a doc site as part of the scaffold so the bets are easily viewable," using Fumadocs (with Mintlify floated as a possible second option). **The generator already exists** (`src/generators/docs-site/`, registered in `generators.json`, mapped in `groundwork-scaffold` phase 01, covered by a generation test). It is *not* a greenfield build — it is an unfinished one. This plan closes the gap between "a Fumadocs project is generated" and "a docs site that actually serves `docs/` well, surfaces bets, runs cleanly, and is offered as a real optional scaffold step." Picks up `G3 docs site` left open in `contract-grade-delivery.md` and builds on the native-runner registry from `dev-cli-native-runners.md`.
...
| ID | Finding | Severity |
|---|---|---|
| D1 | `docs-site` generator exists and is wired, but as a docker-compose service — the wrong primitive for a build that reads the repo-root `docs/` tree | High — architecture |
| D2 | Docker build context (`./services/docs-site` + `COPY . .` in `files/Dockerfile`) cannot see `../../docs`; containerized `next build` compiles against a missing docs dir | High — broken path |
...
Resolves D1, D2, D6. Build on the runner registry from `dev-cli-native-runners.md` (`runners` in `.dev/dev.config.json`, `cli-src/util/runners.ts`).

- **A1 — Self-register a `docs-site` runner instead of joining compose.** In `src/generators/docs-site/generator.ts`, stop injecting into `docker-compose.yml`; instead register a runner (kind `surface`, `autostart: false` by default) into `.dev/dev.config.json`: `cmd: "pnpm dev"` (or `pnpm build && pnpm start`, see open decision O1), `cwd: services/<slug>`, `env: { PORT: <assignedPort> }`. Mirror how `electron-app`/`flutter-app` self-register (WS-C of `dev-cli-native-runners.md`). *Acceptance:* generated `.dev/dev.config.json` contains a `docs-site` runner; `docker-compose.yml` is unchanged; `./dev status` lists the runner.
  - *File:* `src/generators/docs-site/generator.ts` (remove the `composeDoc` service-injection block; add runner registration via the shared helper used by the other surface generators).
...
- **C2 — Hand-off + `infrastructure.md` treatment.** When generated, record the docs site in `docs/infrastructure.md` (service map + how to run it — `./dev start` / the runner) and note it in the scaffold hand-off. When skipped, say nothing. *Acceptance:* a scaffold that includes the docs site documents the run command in `infrastructure.md`; one that skips it does not mention a docs site.
...
**Shipping plumbing (applies on commit):** this is an additive change to an existing optional generator → a `[no-migration]` CHANGELOG line (no existing install requires action). If `cli-src/` changes for runner self-registration, **rebuild the dev bundle** (`npm run build:dev-cli`) — a stale bundle fails the freshness contract test. Run the cheap gates: `./dev test generation && ./dev test contracts`.
