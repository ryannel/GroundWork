...
**A fresh repo flips the burden of proof.** In place, everything survives unless someone kills it. Each deletion is a decision, and decisions can be dodged or half-done. In a new repo nothing exists unless someone puts it there. The delete list stops being arguments to win. It becomes a port list we simply do not write.
...
**Not the old package.** An earlier draft of this plan proposed installing the published GroundWork package into the new repo to deliver the rebuild. That was wrong. Installing GroundWork installs its skills — the package's payload *is* the prose. An agent in that repo would read the old operating contract before anything else. That is the contamination we are trying to escape.

**Not the corpus, in any form.** No copied skills, no lifted templates, no "just as a reference" directory.

**What the quarantine actually protects.** It protects against prose. Reading the old corpus changes how an agent writes, and that is the whole reason for the rule. It is not a rule about mechanical data. A list of file paths, a lint config, or a CI workflow carries no register, so extracting one costs nothing and hiding it helps nobody. The rule is therefore: prose from the old repo never crosses; mechanical facts may, under the same single-dispatch discipline.

**The port pass.** Real assets exist in the old repo and must not be lost: what the CLI test suite asserts, the rules born from real incidents, the four boundary-linter configs, CI parity, and the release pipeline. Extracting those means reading the old repo. That happens in one dispatch with one job: produce a plain-language list of what must remain true. The list is reviewed for register, in a separate session, before it lands. Workers then read the list rather than the source.
...
Seventeen bets, each with a falsifiable done condition. Every one of the spec's 108 commitments is assigned to exactly one of them, checked by script. It lives in [north-star-ladder.md](north-star-ladder.md).
