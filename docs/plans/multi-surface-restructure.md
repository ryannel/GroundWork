...
**Status:** IN EXECUTION 2026-06-12 — WS-A (A1–A5) executed: registry/ledger template + schema, concepts + stack-selection principles docs, spec-format generalization (S9), operating contract confirmed zero-diff. Remaining: WS-B through WS-H. (Plan PROPOSED earlier same day with WS-H, D9–D12, and the review pass folded in.)
...
| O8 | Dart typed-client generation over promoted contracts: codegen tool (e.g. openapi-generator's Dart output) vs a hand-rolled thin client | Decide at H2 with the dated survey; default to the lightest tool that consumes the promoted `openapi.yaml` directly — a heavyweight codegen dependency must earn its place |
