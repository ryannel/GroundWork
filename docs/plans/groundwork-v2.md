...
**W0.4 — Docs refresh.** `docs/groundwork-vs-bmad.md` updated to the v6.9 reading (universal skills architecture, TOML customization rollout + `bmad-customize`, dev-auto, checkpoint-preview, marketplace-plugin modules); the standalone verdict and the 2026-06-10 correction note stay; add a pointer to this plan. *(Executed alongside this plan's landing.)*
...
| `groundwork findings add\|disposition\|check` | W1.4's prose ledger | `check` non-zero on open findings; wired into milestone close; CI-safe like `groundwork-method check` |
| `groundwork decisions add\|pending\|ratify` | W1.6's prose queue | The default+veto machinery |
...
| Wave | Contents | Gate |
|---|---|---|
| 0 | W0.1–W0.4 — independent, ship anytime | `./dev ci` per slice |
