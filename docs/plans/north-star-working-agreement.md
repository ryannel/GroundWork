...
*Draft. The reset commit strips everything above this line. That commit is part of ratification. It runs before any bet — a bet is one self-contained piece of work, built on its own branch. Once the reset commit runs, everything below this line becomes the repo's always-on instruction file. This page is the model for that file: every other page in the rebuild matches its register, its tone and style. So this page must stay plain and short.*
