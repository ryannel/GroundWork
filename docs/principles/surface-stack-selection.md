...
2. **The agent-closable loop.** Can the agent run generate → boot → test → observe end-to-end, without a human driving an IDE, a device, or a signing dialog? GroundWork's sealed-test methodology lives or dies on this axis: a milestone the agent cannot demonstrate autonomously is a milestone a human must babysit, and the delivery loop stalls on every one. A stack whose canonical test harness runs headless in CI beats a stack whose tests need a window and a hand.
