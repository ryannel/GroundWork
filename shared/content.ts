...
/** A product can own a whole repository or selected directory patterns within it. */
const resolvedRepository = (value: string) => repositoryIdentity(value)
...
const repositoryEntryMatches = (entry: ProductRepository, repository: string) =>
  resolvedRepository(entry.repository) === resolvedRepository(repository)
...
/** Membership is derived from product repository declarations. */
export function componentMembership(
  component: Pick<Component, 'repo' | 'sourcePath'>,
  ...
  const owners: string[] = [], used: string[] = []
  for (const product of products) for (const entry of product.repositories ?? []) {
    if (!repository || !repositoryEntryMatches(entry, repository) || !repositoryEntryCovers(entry, component.sourcePath)) continue
    const target = entry.role === 'owned' ? owners : used
    if (!target.includes(product.id)) target.push(product.id)
  }
  const productIds = [...new Set([...owners, ...used])]
  return { ownedBy: owners.length === 1 ? owners[0] : null, usedBy: used,
    productIds, conflictingOwners: owners.length > 1 ? owners : [] }
}
...
export function productRepositories(product: Product, _products: Product[], _components: Component[], _homeRepository?: string): ProductRepository[] {
...
function checker(issues: string[]) {
  return {
    ...
    unique: (values: string[], path: string) => {
      ...
      for (const value of values) { if (seen.has(value)) issues.push(`${path}: duplicate "${value}"`);
        seen.add(value) }
    },
  }
}
...
export function validateProductRepositories(products: Product[]): string[] {
  ...
  for (const product of products) for (const [index, entry] of (product.repositories ?? []).entries()) {
    ...
    if (!parseRemote(entry.repository) && !isProvisional(entry.repository)) issues.push(`${path}: unknown repository identity ${entry.repository}`)
    ...
    if (entry.role === 'owned') {
      for (const pattern of entry.paths?.length ? entry.paths : ['.']) {
        claims.push({ productId: product.id, repository: resolvedRepository(entry.repository), pattern })
      }
    }
  }
  for (let index = 0; index < claims.length; index++) for (const next of claims.slice(index + 1)) {
    ...
    if (claim.productId !== next.productId && claim.repository === next.repository
      ...
      issues.push(`products/${claim.productId}.json and products/${next.productId}.json: `
        + `overlapping ownership of ${claim.repository} (${claim.pattern} and ${next.pattern})`)
    }
  }
  return [...new Set(issues)]
}

/** Catalog integrity for one component: containment, dependencies and execution flows. */
export function validateComponent(component: Component, ctx: ContentContext): string[] {
  ...
  if (component.parentId) {
    ...
    let ancestor = parent
    while (ancestor) {
      if (seen.has(ancestor.id)) { issues.push(`${path}:parentId: containment cycle`);
        break }
      seen.add(ancestor.id);
      ancestor = ctx.componentById.get(ancestor.parentId ?? '')
    }
  }
...
export function validateFeatureSpec(feature: Feature, ctx: ContentContext): string[] {
  ...
  for (const step of sections.journey) {
    refs('flow', step.flow, `journey.json:${step.id}.flow`);
    refs('edges', step.flowEdges, `journey.json:${step.id}.flowEdges`)
    refs('api', step.contracts, `journey.json:${step.id}.contracts`);
    refs('design', step.design ? [step.design] : [], `journey.json:${step.id}.design`)
  ...
  for (const contract of sections.api) {
    componentRef(contract.from, `api.json:${contract.id}.from`);
    componentRef(contract.to, `api.json:${contract.id}.to`)
  ...
  for (const test of sections.tests) {
    refs('journey', test.steps, `tests.json:${test.id}.steps`);
    refs('api', test.contracts, `tests.json:${test.id}.contracts`)
...
export function createRepository(snapshot: ContentSnapshot) {
  ...
  const ancestors = (id: string): Component[] => {
    ...
    path = []
    const seen = new Set<string>()
    for (let c = components.get(id); c && !seen.has(c.id); c = c.parentId ? components.get(c.parentId) : undefined) { seen.add(c.id);
      path.unshift(c) }
  ...
  const q = {
    ...
    touches: (feature: Pick<Feature, 'touches'>, componentId: string) => { const ids = scope(componentId);
      return feature.touches.some(id => ids.has(id)) },
    componentLabel: (id: string) => ancestors(id).map(c => c.name).join(' / '),
    componentType: (id: string) => { const c = components.get(id);
      return c ? componentKindLabel(c) : undefined },
