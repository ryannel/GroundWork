...
export const deliveryEvidenceSchema = z.strictObject({
  id, taskId: id.optional(), validationId: id.optional(), description: text,
...
export const branchLinkSchema = z.strictObject({ branch: text, taskId: id.optional() })
...
export function validateDelivery(featureId: string, plan: Delivery, snapshot: ContentSnapshot) {
  ...
  const units = [...plan.deliverables, ...plan.tasks]
