import { z } from 'zod'
import { commitSha, httpMethodSchema, id, isoTimestamp, text, isRepoRelativePath } from './schema-primitives.ts'
...
export const productSchema = z.strictObject({
  id, workspaceId: id.optional(), slug, order, name: text, kind: productKindSchema,
  description: text.optional(), domain: z.url().optional(), repositories: z.array(productRepositorySchema).optional(),
})
...
export const executionFlowStepSchema = z.strictObject({
  ...
  description: text,
  dataRecordIds: ids.optional(), messageIds: ids.optional(),
  dependencyIds: z.array(z.union([id, z.strictObject({ repository: text, component: id })])).optional(),
...
export const componentSchema = z.strictObject({
  id, order, name: text, kind: componentKindSchema.optional(), parentId: id.optional(),
  dependsOn: z.array(z.union([id, z.strictObject({ repository: text, component: id })])).optional(),
...
export const componentReadSchema = componentSchema
...
export const projectSchema = z.strictObject({ viewerId: id.optional() })
export const documentSchemas = {
  product: productSchema, component: componentSchema, member: memberSchema, feature: featureSchema,
  ...sectionSchemas,
}
