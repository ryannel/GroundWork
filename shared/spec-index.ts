...
const byId = <T extends { id: string }>(xs: T[] | undefined) => { const m = table<T>();
  for (const x of xs ?? []) m[x.id] = x;
  return m }
...
export function buildIndex(spec: FeatureSpec): SpecIndex {
  ...
  for (const e of edges) for (const c of e.contracts ?? []) {
    ix.contractEdge[c] = e.id;
    push(ix.nodeContracts, e.from, c);
      push(ix.nodeContracts, e.to, c)
  }
  ...
  for (const s of steps) {
    ...
    for (const nid of path) {
      ...
      for (const t of ix.node[nid]?.tables ?? []) {
        if (ix.table[t]?.change !== 'removed') { push(ix.stepTables, s.id, t);
          push(ix.tableSteps, t, s.id) }
      }
    }
    ...
    for (const c of crossed) { push(ix.stepContracts, s.id, c);
      push(ix.contractSteps, c, s.id) }
  }
  ...
  for (const t of tests) {
    ...
    for (const sid of t.steps ?? []) {
      ...
      for (const nid of ix.step[sid]?.flow ?? []) { tn.add(nid);
        const c = ix.node[nid]?.component;
        if (c) spans.add(c) }
      for (const cid of ix.stepContracts[sid] ?? []) { const eid = ix.contractEdge[cid];
        if (eid) te.add(eid) }
    }
    for (const cid of t.contracts ?? []) {
      push(ix.contractTests, cid, t.id)
      const c = ix.contract[cid];
      if (c) { spans.add(c.from);
        spans.add(c.to) }
      const eid = ix.contractEdge[cid];
      const edge = eid ? ix.edge[eid] : undefined
      if (eid && edge) { te.add(eid);
        tn.add(edge.from);
        tn.add(edge.to) }
    }
    for (const tid of t.tables ?? []) {
      push(ix.tableTests, tid, t.id)
      const tb = ix.table[tid];
      if (tb) spans.add(tb.component)
      const nid = ix.tableNode[tid];
      if (nid) tn.add(nid)
    }
...
export function refLabel(ix: SpecIndex, r: Ref): { title: string; sub?: string } | undefined {
  switch (r.kind) {
    case 'journey': { const s = ix.step[r.id];
      return s && { title: `Step ${ix.stepOrder.get(r.id)}`, sub: `${s.actor}: ${s.action}` } }
    case 'design': { const m = ix.mockup[r.id];
      return m && { title: m.title, sub: m.kind } }
    case 'flow': { const n = ix.node[r.id];
      return n && { title: n.label, sub: n.kind } }
    case 'api': { const c = ix.contract[r.id];
      return c && { title: c.method ? `${c.method} ${c.path}` : c.path, sub: c.name } }
    case 'storage': { const t = ix.table[r.id];
      return t && { title: t.name, sub: t.change } }
    case 'tests': { const t = ix.test[r.id];
      return t && { title: t.id, sub: t.title } }
    case 'purpose': { const c = ix.criterion[r.id];
